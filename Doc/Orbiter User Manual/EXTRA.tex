...
%TODO add section link
The editor is provided as a plug-in module that comes with the default Orbiter installation. To make it available in Orbiter, make sure that the ScnEditor module is loaded from the \textit{Modules} tab of the Orbiter Launchpad dialog (see TODO).\\
...
%TODO add section link
Below is a short description of the 6 orbital elements. See TODO.
...
\noindent
%TODO add section link
Docking vessels while landed on a planetary surface can sometimes lead to stability problems, in particular if the resulting configuration buries one of the vessels below the surface. See TODO for a workaround.
