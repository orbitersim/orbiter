...
note:set_text(_("This training unit introduces the Attitude Director Indicator instrument for the Shuttle-A. It runs for about 15 minutes. You can fast-forward through any sections with the 'T' and 'R' keys."))
...
note:set_text(_('The new 2-D panel of the Shuttle-A transport vessel has been equipped with a versatile new instrument: the Attitude Director Indicator (or ADI for short).'))

proc.wait_simdt(5)

txt = _('The ADI consists of a ball that represents the orientation of an attitude reference frame with respect to the current vessel orientation.')
note:set_text(txt)
...
txt = _("The markings on the ball represent a grid of pitch and yaw lines ...")
note:set_text(txt);
...
proc.wait_simdt(10)
txt = _("The ring around the ball contains a scale for the roll angle, and the vessel's current roll angle is indicated with a red marker.")
note:set_text(txt)
...
txt = _("The spacecraft's current roll, pitch and yaw angles with respect to the reference frame are also shown on the 'DEV' line in the digital display below the ADI.")
note:set_text(txt)
...
txt = _("You have a choice of attitude reference frames to select from. The most suitable frame depends on the particular maneuver or flight phase you are currently executing.")
note:set_text(txt)
oapi.set_panelblink()

proc.wait_simdt(10)

txt = _("The controls for selecting the reference frame, as well as a number of other functions relating to the attitude reference and ADI operation, are located in the panel below the ADI.")
note:set_text(txt)
...
txt = _("'ECL' stands for ecliptic frame: The plane of the ecliptic is represented by the (pitch=0) plane. The vernal equinox is represented by the (pitch=0,yaw=0) direction, and the ecliptic north pole is represented by the (pitch=+90) direction.")
note:set_text(txt)
v:set_attrefmode(0)
...
txt = _("All of the predefined frames can be modified by adding offsets to the roll, pitch and yaw angles. This will rotate the reference frame against the frame of the base mode.")
note:set_text(txt)
v:set_attrefmode(3)
oapi.set_panelblink()
...
txt = _("Make sure that the 'FRM/TGT' selector switch is set to 'FRM'. This will add the offsets to the reference frame, rather than the target (which will be discussed later).")
note:set_text(txt)
...
txt = _("Use the 'R', 'P' and 'Y' toggles to add or subtract offsets in the roll, pitch and yaw angles.")
note:set_text(txt)
...
txt = _("The current offset angles (in degrees) are shown in the 'OFS' line of the digital display.")
note:set_text(txt)
...
txt = _("To remove the frame offsets, press the 'RESET' button.")
note:set_text(txt)
...
txt = _("The ADI ball also contains two 'error needles' (the horizontal and vertical red bars) which can be used to designate a direction in the reference frame.")
note:set_text(txt)
...
txt = _("The target mode can be selected with the 'TARGET' dial.")
note:set_text(txt)
...
txt = _("'NONE' simply disables the target mode and centers the error needles on the ball.")
note:set_text(txt)
...
txt = _("'FIXED' marks a fixed pitch/yaw point in the reference frame. By default, this is the (pitch=0,yaw=0) direction ...")
note:set_text(txt)
...
txt = _("Make sure that the 'FRM/TGT' selector switch is set to 'TGT'. This will add the offsets to the target direction, rather than the reference frame.")
note:set_text(txt)
oapi.set_panelblink(vec.add(adictrl_ref,{x=8,y=134,z=0}),vec.add(adictrl_ref,{x=30,y=134,z=0}),vec.add(adictrl_ref,{x=8,y=178,z=0}),vec.add(adictrl_ref,{x=30,y=178,z=0}))
...
txt = _("Then use the 'P' and 'Y' toggles to add or subtract offsets to the target direction pitch and yaw angles. Note that the 'R' (roll) toggle is inactive in this mode.")
note:set_text(txt)
...
txt = _("The current target offset angles (in degrees) are shown in the 'OFS' line of the digital display.")
note:set_text(txt)
...
txt = _("As with the reference frame, target offsets can be removed by pressing the 'RESET' button.")
note:set_text(txt)
oapi.set_panelblink(vec.add(adictrl_ref,{x=29,y=134,z=0}),vec.add(adictrl_ref,{x=54,y=134,z=0}),vec.add(adictrl_ref,{x=29,y=178,z=0}),vec.add(adictrl_ref,{x=54,y=178,z=0}))
...
txt = _("Note that the error needles indicate not only the direction towards the target, but also the opposite direction, away from the target. To distinguish between the two, the 'TGT' indicator on the ADI shows 'TO' or 'FROM'.")
note:set_text(txt)
oapi.set_panelblink(vec.add(adicnt,{x=-82,y=-64,z=0}),vec.add(adicnt,{x=-54,y=-64,z=0}),vec.add(adicnt,{x=-82,y=-39,z=0}),vec.add(adicnt,{x=-54,y=-39,z=0}))
...
txt = _("In addition to the fixed target direction, the ADI can also dynamically display the bearing of a NAV radio transmitter. To activate this mode, first set the 'FRAME' selector to NAV 1 or NAV 2, and make sure that the corresponding receiver is tuned to a transmitter in range.")
note:set_text(txt)
oapi.set_panelblink(vec.add(adictrl_ref,{x=57,y=93,z=0}),vec.add(adictrl_ref,{x=85,y=93,z=0}),vec.add(adictrl_ref,{x=57,y=123,z=0}),vec.add(adictrl_ref,{x=85,y=123,z=0}))
...
txt = _("Next, set the 'TARGET' dial to 'N. BRG' (NAV bearing).")
note:set_text(txt)
...
txt = _("When the ADI is slaved to a receiver tuned to an IDS signal, and the target mode is set to bearing information, the ADI indicator acts similarly to the 'Docking' MFD mode, where the ball takes the role of the MFD's attitude alignment indicator (x), and the error needles take the role of the lateral offset indicator (+).")
note:set_text(txt)
oapi.set_panelblink()
...
txt = _("When you have the ball aligned at (R=0,P=0,Y=0), and the error needles are centered, you are located on the correct approach path to the dock, with the correct vessel attitude.")
note:set_text(txt)
...
txt = _("The remaining target mode is 'N. Vel'. Like, N. Brg, this is mode relies on data from a Nav receiver, so the frame mode must be set to Nav 1 or Nav 2, and the associated receiver must be tuned to a transmitter.")
note:set_text(txt)
...
txt = _("This mode is useful for judging your approach towards a dock, landing pad, VOR transmitter, etc.")
note:set_text(txt)

proc.wait_simdt(5)

txt = _("For example, if you are tuned to an IDS or XPDR transmitter, then the N. Vel direction is equivalent to the velocity markers in the Docking HUD mode.")
note:set_text(txt)
...
txt = _("The final component of the ADI are the rate indicators. These are the scales at the top, bottom and right edges of the instrument.")
note:set_text(txt)
...
txt = _("The top scale indicates the current roll rate...")
note:set_text(txt)
oapi.set_panelblink(vec.add(adicnt,{x=-44,y=-84,z=0}),vec.add(adicnt,{x=44,y=-84,z=0}),vec.add(adicnt,{x=-44,y=-64,z=0}),vec.add(adicnt,{x=44,y=-64,z=0}))
v:set_thrustergrouplevel(THGROUP.ATT_BANKLEFT,1)
proc.wait_simdt(3)
...
txt = _("The rates can be displayed in two different frames: Either in the local spacecraft frame, where the rates are directly related to the RCS thrusters for the corresponding axis...")
note:set_text(txt)
...
txt = _("You may come across the ADI ball in two different layouts. So far we have used a ball that uses the pitch=0 plane as the plane of reference, with poles at pitch = +/-90 degrees. The range of yaw angles is 0-360 degrees.")
note:set_text(txt)
proc.wait_simdt(10)

txt = _("An alternative layout uses the yaw = 0 plane as plane of reference, with poles at yaw = +/-90, and pitch range 0-360 degrees.")
note:set_text(txt)
proc.wait_simdt(5)
v:set_adilayout(1)
oapi.set_panelblink(adiball_area[1],adiball_area[2],adiball_area[3],adiball_area[4])
...
txt = _("Note that the two layouts only differ in the markings printed on the ball, not in the way the ball behaves.")
note:set_text(txt)
proc.wait_simdt(7)
...
txt = _("The layout can be selected by adding an ADI_LAYOUT 0/1 entry in the Shuttle-A's scenario definition, or with the V:set_adilayout function in a terminal MFD.")
note:set_text(txt)
proc.wait_simdt(10)

txt = _("This concludes the introduction of the ADI. Got everything? If not, have a look at Doc\\ShuttleA.pdf, section 'Reference frame and ADI'.")
note:set_text(txt)
proc.wait_simdt(7)
...
txt = _("I am going to turn off the MFDs and HUD. You should rely on the ADI only.")
note:set_text(txt)
proc.wait_simdt(5)
v:set_adilayout(0)
  ...
  if mate ~= nil then -- make sure we are docked to ISS
    ...
    if v2:get_name()~='ISS' then
      ...
      if mmate == nil or oapi.get_objname(mmate) ~= 'SH-15' then
        note:set_text(_("Wrong dock! You must dock at port 1 to pass this test."))
        mate = nil
      end
    end        
  end
...
note:set_text(_("Here is the first task: Turn your vessel to a prograde direction and hold."))
step_done = false
t0 = 0
ccount = 0
while not step_done do
  ...
  if v:get_navmode(NAVMODE.PROGRADE) == true then
     ...
     warning = {_("That would be cheating. Please don't push this button again during this task."),_("Do you want to make me angry?"),_("I can fail you for this, you know."),_("You seem to have serious attitude problems ..."),_("That's it, test failed for obnoxious behaviour. If you want to retake it, you'll have to go through the tutorial again.")}
     if ccount < 5 then
       ccount = ccount+1
     end
...
note:set_text(_("Task 1 passed!"))
proc.wait_simdt(5)

-- Test 2: turn to vernal equinox
note:set_text(_("The next task: Turn your vessel towards the vernal equinox and hold. (The vernal equinox is the ascending node of the ecliptic with respect to Earth's equatorial plane.)"))
step_done = false
while not step_done do
  taskSkip()
  step_done = checkTask2()
end
note:set_text(_("Task 2 passed!"))
proc.wait_simdt(5)

-- Test 3: turn to celestial south pole
note:set_text(_("The next task: Turn your vessel towards the celestial south pole and hold. (The celestial south pole is the negative of Earth's spin axis direction.)"))
step_done = false
while not step_done do
  taskSkip()
  step_done = checkTask3()
end
note:set_text(_("Task 3 passed!"))
proc.wait_simdt(5)

-- Test 4: turn towards Sirius
note:set_text(_("Now, turn your vessel towards Sirius. Sirius has the following celestial coordinates: Right ascension 6h 45m 9s, declination -16deg 42' 58''. (You may need pen and paper to convert these coordinates to decimal degrees.)\n\nTip: consider the direction in which right ascension coordinates are increasing."))
step_done = false
while not step_done do
  taskSkip()
  step_done = checkTask4()
end
note:set_text(_("Task 4 passed!"))
proc.wait_simdt(5)

-- Test 5: Dock with ISS
note:set_text(_("All good so far. Here is the last test: Dock with the ISS, docking port 1. (NAV-1 has already been tuned to its IDS transmitter.)"))
step_done = false
while not step_done do
  taskSkip()
  step_done = checkTask5()
end
note:set_text(_("Hard dock! You pass all tests and are now a certified ADI operator. Congratulations!"))
proc.wait_simdt(10)
oapi.del_annotation(note)