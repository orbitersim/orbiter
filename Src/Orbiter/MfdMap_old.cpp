...
SURFHANDLE Instrument_MapOld::LoadBitmap (const char *cbuf, int *w, int *h)
{
	...
	// Load bitmap
	fs::path path = g_pOrbiter->TexPath (cbuf, ".bmp");
	HBITMAP hbm = (HBITMAP)LoadImage (GetModuleHandle(NULL), path.u8string().c_str(), IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
	if (!hbm)
		hbm = (HBITMAP)LoadImage (NULL, path.u8string().c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE|LR_CREATEDIBSECTION);
	if (!hbm)
		return NULL;
	// Get bitmap size
	BITMAP bm;
