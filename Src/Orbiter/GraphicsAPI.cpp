...
OAPIFUNC INT_PTR CALLBACK LaunchpadVideoWndProc(HWND hWnd, UINT uMsg,
...
GraphicsClient::~GraphicsClient() {
    if (hVid)
        SetWindowLongPtr(hVid, GWLP_USERDATA, 0);
    if (splashFont)
...
bool GraphicsClient::clbkInitialise() {
    ...
    ((orbiter::DefVideoTab *)g_pOrbiter->Launchpad()->GetTab(PG_VID))
        ->OnGraphicsClientLoaded(this, fname);
...
int GraphicsClient::clbkVisEvent(OBJHANDLE hObj, VISHANDLE vis, uint32_t msg,
                                 DWORD_PTR context) {
    return 0;
}
...
ParticleStream *
GraphicsClient::clbkCreateParticleStream(PARTICLESTREAMSPEC *pss) {
    return NULL;
}
...
bool GraphicsClient::PlanetTexturePath(const char *planetname,
                                       char *path) const {
...
DWORD GraphicsClient::GetPopupList(const HWND **hPopupWnd) const {
    ...
    if (dlgmgr)
        return dlgmgr->GetDlgList(hPopupWnd);
    else
        return 0;
}
...
SURFHANDLE GraphicsClient::GetVCMFDSurface(int mfd,
                                           const VCMFDSPEC **mfdspec) const {
...
DWORD GraphicsClient::GetBaseTileList(OBJHANDLE hBase,
                                      const SurftileSpec **tile) const {
    return ((Base *)hBase)->GetTileList(tile);
}

// ======================================================================

void GraphicsClient::GetBaseStructures(OBJHANDLE hBase, MESHHANDLE **mesh_bs,
                                       DWORD *nmesh_bs, MESHHANDLE **mesh_as,
                                       DWORD *nmesh_as) const {
    ((Base *)hBase)
        ->ExportBaseStructures((Mesh ***)mesh_bs, nmesh_bs, (Mesh ***)mesh_as,
                               nmesh_as);
}

// ======================================================================

void GraphicsClient::GetBaseShadowGeometry(OBJHANDLE hBase,
                                           MESHHANDLE **mesh_sh, double **elev,
                                           DWORD *nmesh_sh) const {
...
void GraphicsClient::Render2DOverlay() { g_pane->Render(); }

// ======================================================================

bool GraphicsClient::ElevationGrid(ELEVHANDLE hElev, int ilat, int ilng,
                                   int lvl, int pilat, int pilng, int plvl,
                                   INT16 *pelev, float *elev,
                                   double *emean) const {
    if (!hElev)
        return false;
...
void GraphicsClient::ShowDefaultSplash() {
    ...
    // const DWORD bmw = rw, bmh = (rw*10)/16; // source image is 1920x1200,
    // i.e. 16/20 aspect ratio
    ...
    if (bmh < rh)
        tgty = (rh - bmh) / 2;
    if (surf)
        clbkBlt(NULL, tgtx, tgty, surf, 0, 0, bmw, bmh);
...
// Image decoding engine: extract an image from a decoder and rescale it to the
// desired size Return as bitmap
HBITMAP ReadImageFromDecoder(IWICImagingFactory *m_pIWICFactory,
                             IWICBitmapDecoder *piDecoder, UINT w, UINT h) {
    ...
    piScaler->Initialize(piConvertedFrame, w, h,
    ...
    HBITMAP hDIBBitmap = CreateDIBSection(hdcScreen, &bminfo, DIB_RGB_COLORS,
                                          &pvImageBits, NULL, 0);
    ReleaseDC(NULL, hdcScreen);

    piScaler->CopyPixels(NULL, nStride, nImage,
...
HBITMAP GraphicsClient::ReadImageFromMemory(BYTE *pBuf, DWORD nBuf, UINT w,
                                            UINT h) {
    ...
    m_pIWICFactory->CreateDecoderFromStream(
        piStream, NULL, WICDecodeMetadataCacheOnDemand, &piDecoder);

    piStream->Release();
    if (piDecoder) {
        HBITMAP hDIBBitmap =
            ReadImageFromDecoder(m_pIWICFactory, piDecoder, w, h);
        piDecoder->Release();
        return hDIBBitmap;
    } else {
...
HBITMAP GraphicsClient::ReadImageFromFile(const char *fname, UINT w, UINT h) {
    ...
    m_pIWICFactory->CreateDecoderFromFilename(
        wcbuf, NULL, GENERIC_READ, WICDecodeMetadataCacheOnDemand, &piDecoder);
    ...
    if (piDecoder) {
        ...
        LOGOUT_WARN(
            "Couldn't create decoder for image file: %s (does it exist?)",
            fname);
        return 0;
    }
}
...
bool GraphicsClient::WriteImageDataToFile(const ImageData &data,
    ...
    hr = piBitmapFrame->WritePixels(data.height, data.stride, bufsize,
                                    data.data);
...
void GraphicsClient::clbkRender2DPanel(SURFHANDLE *hSurf, MESHHANDLE hMesh,
                                       MATRIX3 *T, bool additive) {
...
void GraphicsClient::clbkRender2DPanel(SURFHANDLE *hSurf, MESHHANDLE hMesh,
                                       MATRIX3 *T, float alpha, bool additive) {
...
bool GraphicsClient::clbkCopyBitmap(SURFHANDLE pdds, HBITMAP hbm, int x, int y,
                                    int dx, int dy) {
    HDC hdcImage;
    HDC hdc;
    BITMAP bm;
...
void GraphicsClient::InitRenderWnd(
    ...
    auto title = std::string(Orbiter::AppTitle) + " " +
...
bool GraphicsClient::RenderWndConsumeEvent(const SDL_Event &event,
                                           bool &wantsOut) {
    return false;
}
...
void GraphicsClient::DrawLaunchpadVideoTab(const WithImCtx &ctx) {}

// ==================================================================
// Functions for the celestial sphere

const std::vector<GraphicsClient::LABELLIST> &
GraphicsClient::GetCelestialMarkers() const {
    return g_psys->LabelList();
}

// ==================================================================

DWORD GraphicsClient::GetSurfaceMarkers(OBJHANDLE hObj,
                                        const LABELLIST **sm_list) const {
    int nlist;
...
DWORD GraphicsClient::GetSurfaceMarkerLegend(OBJHANDLE hObj,
                                             const LABELTYPE **lspec) const {
...
void ParticleStream::Attach(OBJHANDLE hObj, const VECTOR3 *ppos,
                            const VECTOR3 *pdir, const double *srclvl) {
    hRef = hObj;
    SetVariablePos(ppos);
    SetVariableDir(pdir);
    SetLevelPtr(srclvl);
...
void ParticleStream::Attach(OBJHANDLE hObj, const VECTOR3 &_pos,
                            const VECTOR3 &_dir, const double *srclvl) {
    hRef = hObj;
    SetFixedPos(_pos);
    SetFixedDir(_dir);
    SetLevelPtr(srclvl);
...
void ParticleStream::SetVariablePos(const VECTOR3 *ppos) { pos = ppos; }

void ParticleStream::SetVariableDir(const VECTOR3 *pdir) { dir = pdir; }

void ParticleStream::SetLevelPtr(const double *srclvl) { level = srclvl; }
...
void ScreenAnnotation::ClearText() { txtlen = 0; }
...
void ScreenAnnotation::SetSize(double scale) {
    if (scale != txtscale) {
        ...
        // hFont = CreateFont (-hf, 0, 0, 0, 400, 0, 0, 0, 0, 3, 2, 1, 49,
        // "Arial");
    }
}
...
//-----------------------------------------------------------------------
DLLEXPORT LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                   LPARAM lParam) {
    GraphicsClient *gc =
        (GraphicsClient *)GetWindowLongPtr(hWnd, GWLP_USERDATA);
...
DLLEXPORT INT_PTR CALLBACK LaunchpadVideoWndProc(HWND hWnd, UINT uMsg,
                                                 WPARAM wParam, LPARAM lParam) {
    GraphicsClient *gc =
        (GraphicsClient *)GetWindowLongPtr(hWnd, GWLP_USERDATA);
