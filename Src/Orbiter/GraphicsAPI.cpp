...
bool GraphicsClient::clbkInitialise ()
{
	...
	if (clbkUseLaunchpadVideoTab() && g_pOrbiter->Launchpad()) {
		...
		SetWindowLongPtr (hVid, GWLP_USERDATA, (LONG)this);
	} else hVid = NULL;
...
void GraphicsClient::ShowDefaultSplash ()
{
	...
	// do a checksum to make sure the image resource hasn't been tampered with
	BYTE sum[64];
	memset (sum, 0, 64);
	DWORD i;
	for (i = 0; i < nBuf; i++) sum[i%64] += pBuf[i];
#include "splash_cs.dat" // generated by checksplash <path>/Orbiter2016.jpg splash_cs.dat
	// compare image checksum to stored data
	bool ok = (memcmp (chksum, sum, 64) == 0);
	if (!ok) {
		LOGOUT_ERR ("Checksum error. Executable may be corrupt.");
