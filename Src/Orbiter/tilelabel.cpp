...
bool TileLabel::Read()
{
	...
	if (!nlabel && tile->smgr->treeMgr[4]) { // try loading from compressed archive
		BYTE *buf;
		DWORD ndata = tile->smgr->treeMgr[4]->ReadData(lvl+4, ilat, ilng, &buf);
		if (ndata) {
			{
			...
			tile->smgr->treeMgr[4]->ReleaseData(buf);
		}
	}
	return (nlabel > 0);
}
			...
			if (lat >= latmin && lat < latmax && lng >= lngmin && lng < lngmax) {
				...
				if (!alabel[i]->pos.x && !alabel[i]->pos.y && !alabel[i]->pos.z) {
					if (_isnan(alabel[i]->alt))
						alabel[i]->alt = Elevation(lat, lng, latmin, latmax, lngmin, lngmax, tile->mgr->Cbody()->ElevationResolution());
					double rad = tile->mgr->Cbody()->Size() + alabel[i]->alt;
					tile->mgr->Cbody()->EquatorialToLocal(sin(lng), cos(lng), sin(lat), cos(lat), rad, alabel[i]->pos);
				}
			}
		}
	}
