...
void orbiter::LaunchpadDialog2::AddTab(
    std::unique_ptr<orbiter::LaunchpadTab2> tab) {
    tab->GetConfig(m_cfg);
    m_tabList.push_back(std::move(tab));
}

void orbiter::LaunchpadDialog2::Show() {
    m_active = true;
    SDL_ShowWindow(m_window->Inner());
}
...
void orbiter::LaunchpadDialog::InitTabControl(HWND hWnd) {
    ...
    hWait = CreateDialog(hInst, MAKEINTRESOURCE(IDD_PAGE_WAIT2), hWnd,
                         WaitPageProc);
}
...
//-----------------------------------------------------------------------------
INT_PTR orbiter::LaunchpadDialog::DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                          LPARAM lParam) {
    char cbuf[256];
...
void orbiter::LaunchpadDialog::ShowWaitPage(bool show, long mem_committed) {
    ...
    if (show) {
        ...
        ShowWindow(GetDlgItem(hWait, IDC_PROGRESS1),
                   mem_wait ? SW_SHOW : SW_HIDE);
...
void orbiter::LaunchpadDialog::UpdateWaitProgress() {
    if (mem_wait) {
        ...
        SendDlgItemMessage(hWait, IDC_PROGRESS1, PBM_SETPOS,
                           (mem0 - mem) / mem_wait, 0);
    }
}
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK WaitPageProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                              LPARAM lParam) {
