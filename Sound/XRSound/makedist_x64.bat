@rem Assemble the installation package for 64-bit XRSound
@rem Usage: makedist_x64 version_number
@rem   e.g., makedist_x64 3.1
...
set version=%1-x64
if '%version%' NEQ '-x64' GOTO version_ok
echo Usage: makedist_x64 ~version~; e.g., makedist 3.1
goto :eof
...
set irrKlangDLLdir=%IRRKLANG_ROOT_X64%\bin\winx64-visualStudio

echo Packaging 64-bit XRSound %version% release
...
call :copyfile "%SolutionDir%\x64\release\XRSoundDLL.dll" %workdir%\Modules\Plugin\XRSound.dll
call :copyfile "%SolutionDir%\x64\release\XRSound.lib"    %workdir%\Orbitersdk\XRSound\*
call :copyfile "%SolutionDir%\x64\debug\XRSound.lib"      %workdir%\Orbitersdk\XRSound\XRSoundD.lib
