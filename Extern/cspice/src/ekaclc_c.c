...
-Exceptions
   ...
   8)  If the `cvals' input array pointer is null, the error
       SPICE(NULLPOINTER) is signaled.

   9)  If the `cvals' input array strings have length less than two
       characters, the error SPICE(STRINGTOOSHORT) is signaled.

   10) If memory cannot be allocated to create the temporary
...
-Version
   ...
   -CSPICE Version 1.2.3, 10-AUG-2021 (JDR)
       ...
       Added entry #10 to -Exceptions section.
