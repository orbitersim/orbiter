...
/*           velocity components into the local record buffer LOCREC. */
	    ...
	    locrec[(i__2 = to - 1) < 129 && 0 <= i__2 ? i__2 : s_rnge("locrec"
		    , i__2, "spke12_", (ftnlen)258)] = record[from - 1];
	    locrec[(i__2 = to) < 129 && 0 <= i__2 ? i__2 : s_rnge("locrec", 
		    i__2, "spke12_", (ftnlen)259)] = record[from + 2];
	}
...
/*        state. */
	...
	hrmesp_(&n, &record[1], &record[2], locrec, et, work, &state[(i__1 = 
		...
		"spke12_", (ftnlen)267)], &state[(i__2 = i__ + 2) < 6 && 0 <= 
		i__2 ? i__2 : s_rnge("state", i__2, "spke12_", (ftnlen)267)]);
    }
