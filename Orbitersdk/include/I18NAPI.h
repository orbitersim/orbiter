...
// I18NAPI.h
// Internationalisation support interface.
// ======================================================================

/**
 * \file I18NAPI.h
 * \brief Internationalisation support interface.
 */
...
OAPIFUNC const char *PNGetText(uint64_t key, const char *msgid_plural, unsigned int n);

// "Reverse" translate an entity to English (mainly for target selection)
OAPIFUNC const char *GetOriginalName(const char *name);

// Count the number of bytes taken by the first n codepoints
OAPIFUNC std::size_t u8bytes(const char *utf8, std::size_t codepoints) noexcept;
// Number of codepoints in the UTF-8 string
OAPIFUNC std::size_t u8len(const char *utf8) noexcept;
// Copy up to n-codepoints from u8src to u8dst
OAPIFUNC std::size_t u8ncpy(char *u8dst, const char *u8src, std::size_t codepoints, std::size_t dest_size) noexcept;
}; // namespace
