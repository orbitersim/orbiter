...
function AscentApMfd:update_pg_prm(skp)
	...
	if self.vessel.get_status() == 0 then
		skp:text(cw/2, (ch*3)/2, string.format("Launch azimuth: %0.1f°", self.ap.get_launchazimuth()*DEG))
		skp:text(cw/2, (ch*5)/2, string.format("Orbit inc:      %0.1f°", self.ap.get_targetinclination()*DEG))
