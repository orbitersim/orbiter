...
LPDIRECTDRAWSURFACE7 D3D7ParticleStream::deftex = 0;
bool D3D7ParticleStream::bShadows = false;

D3D7ParticleStream::D3D7ParticleStream (GraphicsClient *_gc, PARTICLESTREAMSPEC *pss): ParticleStream (_gc, pss)
{
...
void D3D7ParticleStream::GlobalInit (oapi::D3D7Client *gclient)
{
	gclient->GetTexMgr()->LoadTexture ("contrail1.dds", &deftex, 0);
	bShadows = *(bool*)gclient->GetConfigParam (CFGPRM_VESSELSHADOWS);
}

void D3D7ParticleStream::GlobalExit ()
{
	if (deftex) {
		deftex->Release();
		deftex = 0;
...
void D3D7ParticleStream::SetSpecs (PARTICLESTREAMSPEC *pss)
{
	...
	tex = (pss->tex ? (LPDIRECTDRAWSURFACE7)pss->tex : deftex);
}
...
void D3D7ParticleStream::SetEParticleCoords (const VECTOR3 &ppos, double scale, VERTEX_XYZ_TEX *vtx)
{
	VECTOR3 cdir = ppos;
	double ux, uy, uz, vx, vy, vz, len;
	if (cdir.y || cdir.z) {
...
void D3D7ParticleStream::SetShadowCoords (const VECTOR3 &ppos, const VECTOR3 &cdir, double scale, VERTEX_XYZ_TEX *vtx)
{
	double ux, uy, uz, vx, vy, vz, len;
	if (cdir.y || cdir.z) {
		ux =  0;
