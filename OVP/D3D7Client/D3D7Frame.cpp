...
HRESULT CD3DFramework7::ShowFrame ()
{
    ...
    if (m_bIsFullscreen && m_bPageflip) {
        ...
        // We are in fullscreen mode, so perform a flip.
		HRESULT flipped, hres = m_pddsFrontBuffer->Flip (NULL, m_ddFlipFlag);
		do {
			flipped = m_pddsFrontBuffer->GetFlipStatus(DDGFS_ISFLIPDONE);
		} while (flipped == DDERR_WASSTILLDRAWING); // wait until flip is complete
		return hres;
    } else {
