...
void LogOut_Error (const char *func, const char *file, int line, const char *msg, ...)
{
	...
	oapiWriteLogV (msg, ap);
	va_end(ap);
	oapiWriteLogV ("[%s | %s | %d]", func, file, line);
	oapiWriteLogV ("===============================================================");
}
...
HRESULT LogOut_DDErr (HRESULT hr, const char *func, const char *file, int line) {
	...
	default:								sprintf (err, "DDERR CODE %d", hr); break;
	}
	LogOut ("---------------------------------------------------------------");
	LogOut (errmsg);
...
void LogOut_DIErr (HRESULT hr, const char *func, const char *file, int line) {
	...
	default:								sprintf (err, "DIERR CODE %d", hr); break;
	}
	LogOut ("---------------------------------------------------------------");
	LogOut (errmsg);
	sprintf (logs, ">>> [%s | %s | %d]", func, file, line);
...
void LogOut_DPErr (HRESULT hr, const char *func, const char *file, int line) {
	...
	default:								sprintf (err, "DPERR CODE %d", hr); break;
	}
	LogOut ("---------------------------------------------------------------");
	LogOut (errmsg);
	sprintf (logs, ">>> [%s | %s | %d]", func, file, line);
...
void LogOut_Warning (const char *msg, ...)
{
	va_list ap;
	oapiWriteLogV ("--------------------------- WARNING: --------------------------");
	va_start (ap,msg);
	oapiWriteLogV (msg, ap);
	va_end(ap);
	oapiWriteLogV ("---------------------------------------------------------------");
