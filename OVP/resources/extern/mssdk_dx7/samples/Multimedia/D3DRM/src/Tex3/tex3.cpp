				 ...
				 LPDIRECT3DRMFRAME3 )
{
    ...
    if( FAILED( pD3DRM->CreateWrap( D3DRMWRAP_SPHERE, pFrame,
    ...
    if( FAILED( pD3DRM->LoadTexture( "checker.ppm", &pTex ) ) )
    {
		MSG("Failed to load checker.ppm.\n" );
		goto ret_with_error;
    }
    if( FAILED( pTex->SetColors( 2 ) ) )
	    goto generic_error;
    ...
    if( FAILED( pChild->AddVisual( (LPDIRECT3DRMVISUAL)pMesh ) ) )
	    goto generic_error;
					   ...
					   BOOL* pbConstRenderQuality, CHAR** pstrName )
{
    ...
    (*pstrName) = "Texture Mapping III Direct3DRM Example";
}
