...
//				 2010-2016 Jarmo Nikkanen (vkClient implementation)
// ==============================================================
...
VideoTab::VideoTab(vkClient *gc, HINSTANCE _hInst, HINSTANCE _hOrbiterInst, HWND hVideoTab)
{
	gclient      = gc;
	hInst        = _hInst;
...
BOOL VideoTab::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_VID_INFO:
			DialogBoxParamA(hInst, MAKEINTRESOURCE(IDD_vkSETUP), hTab, SetupDlgProcWrp, (LPARAM)this);
			return TRUE;
		}
		break;
	}
...
INT_PTR CALLBACK VideoTab::SetupDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (LOWORD(wParam)) {
		...
		case IDC_CREDITS:
			...
			DialogBoxParamA(hInst, MAKEINTRESOURCEA(IDD_vkCREDITS), hWnd, CreditsDlgProcWrp, (LPARAM)this);
			break;
...
void VideoTab::CreateSymbolicLinks()
{
	...
	int ret = MessageBox(NULL, "This function will create a symbolic links in /Modules/Server/ folder "
								...
								"Do you want to proceed ?", "vkClient Configuration", MB_YESNO);
	if (ret != IDYES) {
		return;
	}
	...
	MessageBox(NULL, result.c_str(), "vkClient Configuration", MB_OK);
}
...
{
	HANDLE hFile = CreateFile("Modules/vkShaders/Credits.rtf", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL); 

	if (hFile==INVALID_HANDLE_VALUE) {
		LogErr("Failed to open a file /Modules/vkShaders/Credits.rtf");
		return;
	}
	...
	else LogErr("Failed to read a file \\Modules\\vkShaders\\Credits.rtf Error=%u",GetLastError());
