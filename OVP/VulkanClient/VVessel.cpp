...
//				 2010-2019 Jarmo Nikkanen (vkClient modification)
// ==============================================================
...
// ============================================================================================
//
void vVessel::GlobalInit(vkClient *gc)
{
	_TRACE;
	...
	pRenderZone = new ShaderClass(pDevice, "Modules/vkShaders/Custom.hlsl", "QuadVS", "QuadPS", "RenderZones", "");
}
...
// ============================================================================================
//
vkMesh* vVessel::GetMesh (UINT idx)
{
...
void vVessel::LoadMeshes()
{
	...
	for (idx=0;idx<nmesh;idx++) {
		...
		if (hMesh && mesh) {
			...
			meshlist[idx].mesh = new vkMesh(hMesh, *mesh);							// Create new Instance from an existing mesh template
			...
			if (hMesh) {
				...
				meshlist[idx].mesh = new vkMesh(hMesh);								// Create new DX9 Mesh
				meshlist[idx].mesh->SetClass(vClass);
				meshlist[idx].mesh->SetName(idx);
...
void vVessel::InsertMesh(UINT idx)
{
	...
	const vkMesh *mesh = mmgr->GetMesh(hMesh);
...
void vVessel::ErrorOnce(Errors e)
{
	...
	if (bDisp[e]) {
		switch (e) {
		case Errors::NoVC:
			oapiWriteLogV("[ERROR:vk] Cannot identify Virtual Cockpit mesh [%s]", vessel->GetClassNameA());
			break;
		default:
			oapiWriteLogV("[ERROR:vk] Unknows error code [%s]", vessel->GetName());
		}
...
bool vVessel::Render(LPDIRECT3DDEVICE9 dev, const SHADOWMAP *shd, DWORD flg)
{
	...
	HR(vkEffect::FX->SetBool(vkEffect::eEnvMapEnable, false));
	...
	if (shd && shd->IsValid())
	{
		...
		HR(vkEffect::FX->SetMatrix(vkEffect::eLVP, _DX(shd->mLVP))); // Cascade 0 for all
		HR(vkEffect::FX->SetVector(vkEffect::eSHD, _DX(sh)));
		HR(vkEffect::FX->SetVector(vkEffect::eSHDPx, _DX(px)));
		HR(vkEffect::FX->SetBool(vkEffect::eShadowToggle, true));
		vkMesh::SetShadows(shd);
	}
	else {
		vkMesh::SetShadows(NULL);
				...
				if (mFlags & MESHFLAG_VC) {
					if (ec) {
						...
						if (scn->GetRenderPass() != RENDERPASS_MAINSCENE)					
							vkEffect::FX->SetValue(vkEffect::eVCIrrad, &FVECTOR4(b, b, b, 1.0f), sizeof(FVECTOR4));
						else
							vkEffect::FX->SetValue(vkEffect::eVCIrrad, &FVECTOR4(f, f, f, c), sizeof(FVECTOR4));
					}
	...
	if (scn->GetRenderPass() == RENDERPASS_MAINSCENE) {
		if (DebugControls::IsActive()) {
			...
			if (flags&DBG_FLAGS_BOXES && !bInternal) {
				FMATRIX4 id;
				vkEffect::RenderBoundingBox(&mWorld, &FMATRIX_Identity, &BBox.mn, &BBox.mx, ptr(FVECTOR4(1, 0, 0, 0.75f)));
			}
...
void vVessel::RenderClickZones()
{
	...
	for (auto& z : Zones)
	{
		if (z.mode == 2) { // Quadrilateral
			...
			hStockMesh[vkSM_BOX]->RenderGroup(0);
		}
	}
	...
	for (auto& z : Zones)
	{
		if (z.mode == 1) { // Spherical
			...
			pRenderZone->SetPSConstants(hPS, &cb, sizeof(cb));
			pRenderZone->SetVSConstants(hVS, &cb, sizeof(cb));
			hStockMesh[vkSM_SPHERE]->RenderGroup(0);
		}
	}
}
...
// ============================================================================================
//
void vVessel::RenderVectors (LPDIRECT3DDEVICE9 dev, vkPad *pSkp)
{
...
bool vVessel::RenderExhaust()
{
	...
	for (DWORD i=0;i<nexhaust;i++) {
		...
		vkEffect::RenderExhaust(&mWorld, cdir, &es, defexhausttex);
...
void vVessel::RenderGrapplePoints (LPDIRECT3DDEVICE9 dev)
{
	...
	for (i = 0; i < vessel->AttachmentCount(true); ++i)
	{
		...
		vkEffect::RenderArrow(hVessel, &pos, &dir, &rot, size, ptr(FVECTOR4(1.0f, 0.0f, 0.0f, alpha)));
	}
	...
	for (i = 0; i < vessel->AttachmentCount(false); ++i)
	{
		...
		vessel->GetAttachmentParams(hAtt, pos, dir, rot);
		vkEffect::RenderArrow(hVessel, &pos, &dir, &rot, size, ptr(FVECTOR4(0.0f, 0.5f, 1.0f, alpha)));
	}
}
...
void vVessel::RenderLightCone(FMATRIX4* pWT)
{
	...
	if (em->GetType() == LightEmitter::LT_SPOT) {
		...
		WORD Idx[2] = { 0, 1 };
		vkEffect::RenderLines(Main, Idx, 2, 2, pWT, 0xFF00FF00);
		...
		vkEffect::RenderLines(Circle, CIdx, 64, 126, pWT, 0xFF00FF00);
	}	
}
...
void vVessel::AnimateComponent (ANIMATIONCOMP *comp, const FMATRIX4 &T)
{
	...
	if (trans->mesh == LOCALVERTEXLIST) { // transform a list of individual vertices
		...
		vkMesh *mesh = meshlist[trans->mesh].mesh;
		if (!mesh) return;
...
void vVessel::RenderReentry(LPDIRECT3DDEVICE9 dev)
{
	...
	vkEffect::RenderReEntry(defreentrytex, &vPosA, &vPosB, &vDir, alpha_A, alpha_B, size);
}
...
// ===========================================================================================
//
vkPick vVessel::Pick(const FVECTOR3 *vDir, const PickProp *p)
{
	FMATRIX4 mWT;
	FMATRIX4* pWT = NULL;
	...
	for (DWORD i=0;i<nmesh;i++) {
		...
		vkPick pick = hMesh->Pick(&mWorld, meshlist[i].trans, vDir, p);
