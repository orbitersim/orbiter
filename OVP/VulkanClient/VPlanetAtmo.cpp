...
// ===========================================================================================
//
void vPlanet::GlobalInitAtmosphere(oapi::vkClient* gc)
{
	pDev = gc->GetDevice();

	if (Config->bAtmoQuality) pIP = new ImageProcessing(pDev, "Modules/vkShaders/Scatter.hlsl", "SunColor");
	else pIP = new ImageProcessing(pDev, "Modules/vkShaders/Scatter.hlsl", "SunColor", "_PERFORMANCE");
	...
	pRender[PLT_GIANT] = new PlanetShader(pDev, "Modules/vkShaders/NewPlanet.hlsl", "GiantVS", "GiantPS", "Giant", flags.c_str());
	pRender[PLT_G_CLOUDS] = new PlanetShader(pDev, "Modules/vkShaders/NewPlanet.hlsl", "CloudVS", "GiantCloudPS", "GiantCloud", flags.c_str());

	if (Config->CloudMicro) flags += "_CLOUDMICRO ";
	if (Config->bCloudNormals) flags += "_CLOUDNORMALS ";

	pRender[PLT_CLOUDS] = new PlanetShader(pDev, "Modules/vkShaders/NewPlanet.hlsl", "CloudVS", "CloudPS", "Clouds", flags.c_str());
	...
	pRender[PLT_MARS] = new PlanetShader(pDev, "Modules/vkShaders/NewPlanet.hlsl", "TerrainVS", "TerrainPS", "Mars", (flags + blend).c_str());


	flags += "_NO_ATMOSPHERE ";

	pRender[PLT_MOON] = new PlanetShader(pDev, "Modules/vkShaders/NewPlanet.hlsl", "TerrainVS", "TerrainPS", "Moon", (flags + blend).c_str());
	...
	pRender[PLT_EARTH] = new PlanetShader(pDev, "Modules/vkShaders/NewPlanet.hlsl", "TerrainVS", "TerrainPS", "Earth", (flags + blend).c_str());
}
...
{
	float d = dotp(cp.toCam, cp.toSun);
	float q = cp.CamRad * d;
...
{
	float d = 0.0f;
	float a = dotp(vNrm, vRay);
	if (!CameraInAtmosphere()) d = RayLength(a, r);
	else d = abs(dotp(vPos - cp.CamPos, vRay));
...
void vPlanet::IntegrateSegment(FVECTOR3 vOrig, FVECTOR3 vRay, float len, FVECTOR4* ral, FVECTOR4* mie, FVECTOR4* tot)
{
	...
	for (int i = 0; i < NSEG; i++)
	{
		...
		FVECTOR3 x = SunLightColor(-dotp(n, cp.toSun), alt) * ComputeCameraView(-ang, rad, len - dst).rgb;
...
float vPlanet::SunOcclusionByPlanet()
{
	...
	double r = dotp(up, -cpos);
	double ca = dotp(up, SunDirection());
...
// ===========================================================================================
//
vkSun vPlanet::GetObjectAtmoParams(VECTOR3 vRelPos)
{
	...
	float dNR = dotp(vNrm, vRay);			// cosine of (Normal/vRay) angle 
	float dRS = dotp(vRay, cp.toSun);
	float dNS = dotp(vNrm, cp.toSun);	
	float alt = r - size;
...
void vPlanet::UpdateScatter()
{
	...
	cp.dCS = dotp(cp.toCam, cp.toSun);
	cp.ZeroAz = unit(crossp(cp.toCam, cp.toSun));	// Sun-aligned Atmo Scatter RefFrame
	cp.SunAz = unit(crossp(cp.toCam, cp.ZeroAz));	// Sun-aligned Atmo Scatter RefFrame
	cp.Up = unit(crossp(cp.ZeroAz, cp.toSun));
	...
	float A = dotp(cp.toCam, cp.Up) * cp.CamRad;
	cp.Cr2 = A * A;
	...
	sFlow Flow;
	Flow.bCamLit = !((cp.Cr2 < cp.PlanetRad2) && (dotp(cp.toCam, cp.toSun) < 0));
...
vPlanet::SHDPrm vPlanet::ComputeShadow(FVECTOR3 vRay)
{
	...
	double A = dotp(TestPrm.Up, TestPrm.toCam * TestPrm.CamRad);
	sp.cr = abs(A);

	// Projection of viewing ray on 'shadow' axes
	double u = dotp(vRay, TestPrm.Up);
	double t = dotp(vRay, TestPrm.ZeroAz);
	double z = dotp(vRay, TestPrm.toSun);
	...
	if (w2 < 0) {
		...
		if (dotp(vEn, TestPrm.toSun) > 0) sp.se = invalid_val;
		if (dotp(vEx, TestPrm.toSun) > 0) sp.sx = invalid_val;
	}
...
void vPlanet::TestComputations(Sketchpad* pSkp)
{
	...
	if (length(GetScene()->vPickRay) > 0.8f) {
		vRef = campos;
		beta = dotp(unit(vRef), GetScene()->vPickRay);
		...
		TestPrm.ZeroAz = unit(crossp(TestPrm.toCam, TestPrm.toSun));
		TestPrm.SunAz = unit(crossp(TestPrm.toCam, TestPrm.ZeroAz));
		TestPrm.Up = unit(crossp(TestPrm.ZeroAz, TestPrm.toSun));
	...
	vkDebugLog("Optical Depth=%f   RGB(%f, %f, %f)", rd.a, rd.r, rd.g, rd.b);

	float sl = dotp(cp.toSun, TestPrm.toCam);
	float sa = dotp(vRay, TestPrm.toCam);
	float rl = RayLength(-sa, TestPrm.CamRad);
	float rf = sqrt(cp.AtmoRad2 - cp.PlanetRad2);
	float ph = dotp(vRay, cp.toSun);
	...
	vkDebugLog("Sunlight RGB(%f, %f, %f)", cl.x, cl.y, cl.z);
	vkDebugLog("InScatter RGB(%f, %f, %f)", is.x, is.y, is.z);
	vkDebugLog("RayLength = %f vs %f, sa = %f, sl = %f", rl, rf, sa, sl);
	vkDebugLog("AtmoAlt = %f(km)", cp.AtmoAlt/1000.0f);

	double u = dotp(vPos, TestPrm.Up);
	double t = dotp(vPos, TestPrm.ZeroAz);
	bool bTgt = ((u * u + t * t) < cp.PlanetRad2 && dotp(vPos, TestPrm.toSun) < 0);
	bool bSrc = (sp.cr < cp.PlanetRad&& dotp(vRef, TestPrm.toSun) < 0);
	...
	if (bSrc) vkDebugLog("Camera in Shadow");
	if (bTgt) vkDebugLog("Target in Shadow");


	if (status == 1) vkDebugLog("SURFACE");
	if (status == 2) vkDebugLog("HORIZON");
	if (status == 3) vkDebugLog("SKYDOME");

	vkDebugLog("Shadow First=%f, Second=%f", sp.se, sp.sx);
	vkDebugLog("Atmosp First=%f, Second=%f", sp.ae, sp.ax);
	vkDebugLog("Hd=%f, Ca=%f", sp.hd, sp.ca);
	vkDebugLog("CameraInSpace=%f", cp.CamSpace);

	FMATRIX4 mI; oapiMatrixIdentity(&mI);
	VECTOR3 V0, V1;
	IVECTOR2 pt0, pt1;
	...
	vkDebugLog("s0=%f, e0=%f, s1=%f, e1=%f, mp=%f, lf=%f", s0, e0, s1, e1, mp, lf);

	if (e0 > s0) {
		vkDebugLog("Primary Integral");
		V0 = (vR + vRay * s0)._V();
