...
SURFHANDLE vkParticleStream::deftex = 0;
SURFHANDLE vkParticleStream::deftexems = 0;
bool vkParticleStream::bShadows = false;

vkParticleStream::vkParticleStream(GraphicsClient *_gc, PARTICLESTREAMSPEC *pss) : ParticleStream (_gc, pss), vkEffect()
{
	pGC = (vkClient*)_gc;
...
vkParticleStream::~vkParticleStream()
{
	while (pfirst) {
...
void vkParticleStream::GlobalInit (oapi::vkClient *gclient)
{
...
void vkParticleStream::GlobalExit ()
{
	DELETE_SURFACE(deftex);
	DELETE_SURFACE(deftexems);
}

void vkParticleStream::SetSpecs(PARTICLESTREAMSPEC *pss)
{
...
void vkParticleStream::SetParticleHalflife (double pht)
{
...
void vkParticleStream::SetObserverRef (const VECTOR3 *cam)
{
	LogErr("vkParticleStream::SetObserverRef() NOT IMPLEMENTED");
	//cam_ref = cam;
}

void vkParticleStream::SetSourceRef (const VECTOR3 *src)
{
	LogErr("vkParticleStream::SetSourceRef() NOT IMPLEMENTED");
	//src_ref = src;
}

void vkParticleStream::SetSourceOffset (const VECTOR3 &ofs)
{
	LogErr("vkParticleStream::SetSourceOffset() NOT IMPLEMENTED");
	//src_ofs = ofs;
}

void vkParticleStream::SetIntensityLevelRef (double *lvl)
{
	level = lvl;
}

double vkParticleStream::Level2Alpha(double level) const
{
	switch (lmap) {
...
double vkParticleStream::Atm2Alpha(double prm) const
{
	switch (amap) {
...
ParticleSpec *vkParticleStream::CreateParticle (const VECTOR3 &pos, const VECTOR3 &vel, double size, double alpha)
{
...
void vkParticleStream::DeleteParticle (ParticleSpec *p)
{
...
void vkParticleStream::Timejump()
{
	while (pfirst) {
		ParticleSpec *tmp = pfirst;
		pfirst = pfirst->next;
...
void vkParticleStream::SetDParticleCoords(const VECTOR3 &ppos, double scale, NTVERTEX *vtx)
{
	VECTOR3 cdir = ppos;
...
void vkParticleStream::SetEParticleCoords (const VECTOR3 &ppos, double scale, VERTEX_XYZ_TEX *vtx)
{
	VECTOR3 cdir = ppos;
	double ux, uy, uz, vx, vy, vz, len;
	if (cdir.y || cdir.z) {
...
void vkParticleStream::SetShadowCoords(const VECTOR3 &ppos, const VECTOR3 &cdir, double scale, VERTEX_XYZ_TEX *vtx)
{
	double ux, uy, uz, vx, vy, vz, len;
...
void vkParticleStream::CalcNormals(const VECTOR3 &ppos, NTVERTEX *vtx)
{
...
void vkParticleStream::Render(LPDIRECT3DDEVICE9 dev)
{
	if (!pfirst) return;
...
void vkParticleStream::RenderDiffuse(LPDIRECT3DDEVICE9 dev)
{
...
void vkParticleStream::RenderEmissive(LPDIRECT3DDEVICE9 dev)
{
	static D3DMATERIAL9 smokemat = { // emissive material for engine exhaust
		{0,0,0,1},
		{0,0,0,1},
