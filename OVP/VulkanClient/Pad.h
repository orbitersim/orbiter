...
typedef std::shared_ptr<vkText> vkTextPtr;
...
/**
 * \brief The vkPad class defines the context for 2-D drawing using
 *  DirectX calls.
 */
class vkPad : public Sketchpad
{
	friend class vkText;
...
public:
	 ...
	 */
	explicit vkPad(SURFHANDLE s, const char *name = NULL);
	explicit vkPad(const char *name = NULL);
	 ...
	 *   Usually this should be set to "vkClient")
	 */
	static void vkTechInit(vkClient *gc, LPDIRECT3DDEVICE9 pDev);
	...
	*  - All the other vkPad functions can be only called/used between the BeginDrawing(), EndDrawing() pairs.
	*  - All rendering that is NOT a part of vkPad is prohibited between the Begin/End pairs.
	*  - Calling Begin/End doesn't alter the vkPad state such as Pens, Brushes, etc...
...
class vkPadFont: public oapi::Font {
...
public:
	...
	static void vkTechInit(LPDIRECT3DDEVICE9 pDev);
	 ...
	 */
	vkPadFont (int height, bool prop, const char *face, FontStyle style = FontStyle::FONT_NORMAL, int orientation=0, DWORD flags=0);
	vkPadFont (int height, char *face, int width = 0, int weight = 400, FontStyle style = FontStyle::FONT_NORMAL, float spacing = 0.0f);
	/**
...
class vkPadPen: public oapi::Pen {
...
public:
	 ...
	 */
	vkPadPen (int style, int width, DWORD col);
...
class vkPadBrush: public oapi::Brush {
	friend class vkPad;
	friend class GDIPad;
...
class vkPolyBase
{
	DWORD alloc_id;
public:
					vkPolyBase(int _type) : alloc_id('POLY') { type = _type; version = 1; }
	virtual			~vkPolyBase() { }
	virtual	void	Draw(vkPad*, LPDIRECT3DDEVICE9 pDev) = 0;
...
class vkPolyLine : public vkPolyBase
{

public:
	vkPolyLine(LPDIRECT3DDEVICE9 pDev, const FVECTOR2 *pt, int npt, bool bConnect);
	~vkPolyLine();

	void Update(const FVECTOR2 *pt, int npt, bool bConnect);
	void Draw(vkPad*, LPDIRECT3DDEVICE9 pDev);
	void Release();
...
class vkTriangle : public vkPolyBase
{

public:
	vkTriangle(LPDIRECT3DDEVICE9 pDev, const gcCore::clrVtx *pt, int npt, int style);
	~vkTriangle();
