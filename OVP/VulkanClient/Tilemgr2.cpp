...
void Tile::Render()
{
	if (mgr->gc->IsControlPanelOpen())
		if (vkStats.TilesRendered.count(mgr->GetScene()->GetRenderPass()))
			vkStats.TilesRendered[mgr->GetScene()->GetRenderPass()]++;
}
...
TileLoader::TileLoader (const oapi::vkClient *gclient)
	: gc(gclient)
...
oapi::vkClient* TileManager2Base::gc = NULL;
...
void TileManager2Base::GlobalInit (class oapi::vkClient *gclient)
{
	gc = gclient;
	pDev = gc->GetDevice();
