...
{
	vkDebugLog("[%3.3f, %3.3f, %3.3f, %3.3f]", pM->m41, pM->m42, pM->m43, pM->m44);
	vkDebugLog("[%3.3f, %3.3f, %3.3f, %3.3f]", pM->m31, pM->m32, pM->m33, pM->m34);
	vkDebugLog("[%3.3f, %3.3f, %3.3f, %3.3f]", pM->m21, pM->m22, pM->m23, pM->m24);
	vkDebugLog("[%3.3f, %3.3f, %3.3f, %3.3f]", pM->m11, pM->m12, pM->m13, pM->m14);
	vkDebugLog("%s", name);
}
...
// ===========================================================================================
//
Scene::Scene(vkClient *_gc, DWORD w, DWORD h)
{
	_TRACE;
	...
	m_celSphere = new vkCelestialSphere(gc, this);
	Lights = new vkLight[MAX_SCENE_LIGHTS];

	bLocalLight = *(bool*)gc->GetConfigParam(CFGPRM_LOCALLIGHT);
	
	memset(&sunLight, 0, sizeof(vkSun));
	...
	ifstream fs("Modules/vkShaders/GKernel.txt");
	if (fs.good()) {
		...
		if (data.size() != ARRAYSIZE(DepthSampleKernel)) LogErr("Modules/vkShaders/GKernel.txt Size missmatch. Expecting 57 entries");
		else for (int i = 0; i < ARRAYSIZE(DepthSampleKernel); i++) DepthSampleKernel[i] = data[i];
		data.clear();
	} else LogErr("Failed to read: Modules/vkShaders/GKernel.txt");
	fs.close();
	...
	//
	dmCubeMesh = (DEVMESHHANDLE) new vkMesh("D3D9Cube");
	pRenderStage = new ShaderClass(pDevice, "Modules/vkShaders/Custom.hlsl", "StageVS", "StagePS", "RenderStage", "");
	...
	{
		pRenderGlares = new ShaderClass(pDevice, "Modules/vkShaders/Glare.hlsl", "GlareVS", "GlarePS", "RenderGlares", "");
		pLocalCompute = new ShaderClass(pDevice, "Modules/vkShaders/Glare.hlsl", "VisibilityVS", "VisibilityPS", "LocalVisCheck", "");
	...
	if (Config->bGlares || Config->bLocalGlares) {
		pVisDepth = new ImageProcessing(pDevice, "Modules/vkShaders/LightBlur.hlsl", "PSDepth", NULL);
	...
	if (Config->GDIOverlay) {
		...
		pGDIOverlay = new ImageProcessing(pDevice, "Modules/vkShaders/GDIOverlay.hlsl", "PSMain");
	}
	...
	if (Config->PostProcess) {
		...
		if (Config->PostProcess == PP_DEFAULT)
			pLightBlur = new ImageProcessing(pDevice, "Modules/vkShaders/LightBlur.hlsl", "PSMain", flags);
	...
	pBakeLights = new ImageProcessing(pDevice, "Modules/vkShaders/PreBakeLights.hlsl", "PSMain");
...
void Scene::CreateSunGlare()
{
	...
	pCreateGlare = new ImageProcessing(pDevice, "Modules/vkShaders/Glare.hlsl", "CreateSunGlarePS");
...
//
vkPad *Scene::GetPooledSketchpad (int id) // one of SKETCHPAD_xxx
{
	assert(id <= SKETCHPAD_PLANETARIUM);

	if (!_pad) _pad = new vkPad("POOLED_SKETCHPAD");
...
// ===========================================================================================
//
const vkLight *Scene::GetLight(int index) const
{
...
void Scene::clbkUpdate ()
{
	...
	if (hTgt!=Camera.hTarget && hTgt!=NULL) {
		...
		if (vo) {
			...
			//         vkClient does a oapiSetFocusObject call when playback
...
bool Scene::UpdateCamVis()
{
	...
	if (Camera.hObj_proxy) vkEffect::UpdateEffectCamera(Camera.hObj_proxy);
...
void Scene::clbkRenderMainScene()
{
	...
	double scene_time = vkGetTime();
	vkSetTime(vkStats.Timer.CamVis, scene_time);
		...
		if (Config->CustomCamMode && (CustomCams.size() > 0))
		{
			...
			if (vO->CamDist() < maxd && (*camCurrent)->bActive)
			{
				...
				if ((*camCurrent)->pRenderProc) {
					vkPad *pSkp = (vkPad * )gc->clbkGetSketchpad((*camCurrent)->hSurface);
					pSkp->LoadDefaults();
	...
	for (auto pl : Planets)
	{
		...
		vkPad *pSketch = GetPooledSketchpad(SKETCHPAD_LABELS);
	...
	if (plnmode & PLN_ENABLE) {
		vkPad *pSketch = GetPooledSketchpad(SKETCHPAD_PLANETARIUM);
	...
	vkEffect::UpdateEffectCamera(Camera.hObj_proxy);
	...
	if (oapiCameraInternal() && vFocus && (oapiCockpitMode() == COCKPIT_VIRTUAL))
	{
		...
		if (Config->ShadowMapMode >= 1)
		{
			...
			//vkDebugLog("Aperture = %f, dist=%f", GetCameraApertureCorner() * 2.0 * 180.0 / PI, cascfg[2].dist- cascfg[2].size);
	...
	/*if (Camera.vNear) {
		vkDebugLog("vNear = %s", Camera.vNear->GetName());
		vkDebugLog("vProxy = %s", Camera.vProxy->GetName());
		vkDebugLog("vGravRef = %s", Camera.vGravRef->GetName());
	}*/
	...
	if (len>0 || !vkDebugQueue.empty()) {
		...
		//
		while (!vkDebugQueue.empty()) {
			pos -= (height * 3) / 2;
			std::string str = vkDebugQueue.front();
...
// ===========================================================================================
//
LPDIRECT3DTEXTURE9 Scene::RenderObjectsInShadow(SMapInput* smi, list<vVessel*>& rList, vkPad* pSkp)
{
...
// ===========================================================================================
//
void Scene::RenderVesselMarker(vVessel *vV, vkPad *pSketch)
{
	DWORD mkrmode = *(DWORD*)gc->GetConfigParam(CFGPRM_SURFMARKERFLAG);
...
Scene::SUNVISPARAMS Scene::GetSunScreenVisualState()
{
	...
	vkPick pick = PickScene(xpos, ypos, &prp);
...
#ifdef CASCADE_DEBUG
		vkDebugLog("Cascade %i  pos=[%f, %f]", i, xy.x, xy.y);
		vkDebugLog("Cascade %i  rect=[%f, %f, %f, %f]", i, xy.x-r, xy.y-r, xy.x+r, xy.y+r);
#endif
	...
	const LPDIRECT3DCUBETEXTURE9 pCT, SHADOWMAP *sm)
{
	...
	vkEffect::UpdateEffectCamera(GetCameraProxyBody());
...
void Scene::RenderStage(LPDIRECT3DCUBETEXTURE9 pCT)
{
	...
	((vkMesh*)dmCubeMesh)->RenderGroup(0);
...
bool Scene::RenderBlurredMap(LPDIRECT3DDEVICE9 pDev, LPDIRECT3DCUBETEXTURE9 pSrc)
{
	...
	if (!pBlur) {
		pBlur = new ImageProcessing(pDev, "Modules/vkShaders/EnvMapBlur.hlsl", "PSBlur");
	}
...
bool Scene::RenderBlurredMap(LPDIRECT3DDEVICE9 pDev, LPDIRECT3DTEXTURE9 pSrc)
{
	...
	if (!pBlur2D) {
		pBlur2D = new ImageProcessing(pDev, "Modules/vkShaders/EnvMapBlur.hlsl", "PS2DBlur");
	}
...
bool Scene::IntegrateIrradiance(vVessel *vV, ENVCAMREC *ec, bool bInterior)
{
	...
	if (!pIrradiance) {
		pIrradiance = new ImageProcessing(pDevice, "Modules/vkShaders/IrradianceInteg.hlsl", "PSInteg");
...
void Scene::VisualizeShadowMap(SHADOWMAP *sm)
{
	...
	vkPad *pSketch = GetPooledSketchpad(SKETCHPAD_2D_OVERLAY);
...
{
	vkMesh *pMesh = (vkMesh *)hMesh;

	float s = float(smEX->size);
	float sr = 2.0f * smEX->rad / s;

	HR(vkEffect::FX->SetMatrix(vkEffect::eLVP, _DX(smEX->mLVP)));

	if (smEX->IsValid()) {
		pMesh->SetShadows(smEX);
		HR(vkEffect::FX->SetVector(vkEffect::eSHD, _DX(FVECTOR4(sr, 1.0f / s, float(oapiRand()), 1.0f / smEX->depth))));
		HR(vkEffect::FX->SetBool(vkEffect::eShadowToggle, true));
	}
	else {
		pMesh->SetShadows(NULL);
		HR(vkEffect::FX->SetBool(vkEffect::eShadowToggle, false));
	}
...
// ===========================================================================================
//
void Scene::AddParticleStream (class vkParticleStream *_pstream)
{

	vkParticleStream **tmp = new vkParticleStream*[nstream+1];
	if (nstream) {
		memcpy (tmp, pstream, nstream*sizeof(vkParticleStream*));
		delete []pstream;
	}
	pstream = tmp;
...
void Scene::DelParticleStream (DWORD idx)
{
	...
	vkParticleStream **tmp;
	if (nstream > 1) {
		DWORD i, j;
		tmp = new vkParticleStream*[nstream-1];
...
// ===========================================================================================
//
vkPick Scene::PickScene(short xpos, short ypos, const PickProp *p)
{
...
// ===========================================================================================
//
vkPick Scene::PickMesh(DEVMESHHANDLE hMesh, const FMATRIX4* pW, short xpos, short ypos)
{
	vkMesh *pMesh = (vkMesh *)hMesh;
	PickProp prp = { NULL, 0.1f, false };
...
void Scene::SetCameraAperture(float ap, float as)
{
	...
	vkEffect::SetViewProjMatrix(&Camera.mProjView);
}
...
// ===========================================================================================
//
void Scene::vkTechInit(LPDIRECT3DDEVICE9 pDev, const char *folder)
{
	char name[256];
	...
	vkCelestialSphere::vkTechInit(FX);
}
