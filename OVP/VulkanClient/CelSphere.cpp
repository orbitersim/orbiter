...
vkCelestialSphere::vkCelestialSphere(vkClient *gc, Scene *scene)
...
vkCelestialSphere::~vkCelestialSphere()
{
	ClearStars();
	m_clVtx->Release();
	m_cbVtx->Release();
...
void vkCelestialSphere::InitCelestialTransform()
{
...
bool vkCelestialSphere::LocalHorizonTransform(MATRIX3& R, FMATRIX4& T)
{
	MATRIX3 rot;
...
void vkCelestialSphere::InitStars ()
{
	ClearStars();
...
void vkCelestialSphere::ClearStars()
{
	for (auto it = m_sVtx.begin(); it != m_sVtx.end(); it++)
		(*it)->Release();
	m_sVtx.clear();
...
int vkCelestialSphere::MapLineBuffer(const std::vector<VECTOR3>& lineVtx, LPDIRECT3DVERTEXBUFFER9& buf) const
{
...
void vkCelestialSphere::InitConstellationLines()
{
...
void vkCelestialSphere::InitConstellationBoundaries()
{
...
void vkCelestialSphere::AllocGrids ()
{
	int i, j, idx;
	double lng, lat, xz, y;
	VERTEX_XYZ *vbuf;
...
void vkCelestialSphere::AllocGridLabels()
{
	VERTEX_XYZ_TEX* vbuf;
	WORD* ibuf;
...
void vkCelestialSphere::OnOptionChanged(DWORD cat, DWORD item)
{
	switch (cat) {
...
void vkCelestialSphere::Render(LPDIRECT3DDEVICE9 pDevice, const VECTOR3& skyCol)
{
	SetSkyColour(skyCol);
	...
	if (renderFlag & PLN_ENABLE) {
		...
		vkPad* pSketch = m_scene->GetPooledSketchpad(0);
...
void vkCelestialSphere::RenderStars(ID3DXEffect *FX)
{
	_TRACE;
...
void vkCelestialSphere::RenderConstellationLines(ID3DXEffect *FX)
{
...
void vkCelestialSphere::RenderConstellationBoundaries(ID3DXEffect* FX)
{
...
void vkCelestialSphere::RenderGreatCircle(ID3DXEffect *FX)
{
	_TRACE;
	UINT numPasses = 0;
	HR(FX->Begin(&numPasses, D3DXFX_DONOTSAVESTATE));
...
void vkCelestialSphere::RenderGrid(ID3DXEffect *FX, bool eqline)
{
	_TRACE;
	int i;
	UINT numPasses = 0;
...
void vkCelestialSphere::RenderGridLabels(ID3DXEffect* FX, int az_idx, const oapi::FVECTOR4& baseCol, const MATRIX3& R, double dphi)
{
...
void vkCelestialSphere::RenderBkgImage(LPDIRECT3DDEVICE9 dev)
{
...
bool vkCelestialSphere::EclDir2WindowPos(const VECTOR3& dir, int& x, int& y) const
{
...
void vkCelestialSphere::vkTechInit(ID3DXEffect* fx)
{
	s_FX = fx;
...
ID3DXEffect* vkCelestialSphere::s_FX = 0;
D3DXHANDLE vkCelestialSphere::s_eStar = 0;
D3DXHANDLE vkCelestialSphere::s_eLine = 0;
D3DXHANDLE vkCelestialSphere::s_eLabel = 0;
D3DXHANDLE vkCelestialSphere::s_eColor = 0;
D3DXHANDLE vkCelestialSphere::s_eWVP = 0;
