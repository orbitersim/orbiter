...
public:
	...
	MeshShader(LPDIRECT3DDEVICE9 pDev, const char *file, const char *vs, const char *ps, const char *opt = NULL) :
		ShaderClass(pDev, "Modules/vkShaders/NewMesh.hlsl", vs, ps, "MeshShader", opt)
	{
...
public:
	...
	MeshBuffer(MeshBuffer *pSrc, const class vkMesh *_pRoot);
	MeshBuffer(DWORD nVtx, DWORD nIdx, const class vkMesh *_pRoot);
	~MeshBuffer();

	void Map(LPDIRECT3DDEVICE9 pDev);
	bool IsLocalTo(const class vkMesh *_pRoot) const { return (_pRoot == pRoot); }
...
/**
 * \brief Mesh object with vk-specific vertex buffer
 *
 * Meshes consist of one or more vertex groups, and a set of materials and
 * textures.
 */

class vkMesh : private vkEffect
{
...
public:
	...
	struct GROUPREC {			// mesh group definition
		...
		float TexMixEx[MAXTEX];
					...
					vkMesh(const char *fname);
					vkMesh(DWORD nGrp, const MESHGROUPEX **hGroup, const SURFHANDLE *hSurf);
					vkMesh(const MESHGROUPEX *pGroup, const MATERIAL *pMat, SurfNative *pTex);
					vkMesh(MESHHANDLE hMesh, bool asTemplate = false, FVECTOR3 *reorig = NULL, float *scale = NULL);
					vkMesh(MESHHANDLE hMesh, const vkMesh &hTemp);
					~vkMesh();
	 ...
	 */
	const vkMatExt *	GetMaterial(DWORD idx) const;
	bool			GetMaterial(vkMatExt *pMat, DWORD idx) const;
	void			SetMaterial(const vkMatExt *pMat, DWORD idx, bool bUpdateStatus = true);
	...
	void			SetSunLight(const vkSun *pLight);

	vkPick		Pick(const FMATRIX4* pW, const FMATRIX4* pT, const FVECTOR3 *vDir, const PickProp* p);
...
private:
	...
	DWORD vClass;
	DWORD Flags;
	vkMatExt *Mtrl;           // list of mesh materials
