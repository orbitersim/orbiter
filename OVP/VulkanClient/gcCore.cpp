...
{
	LPDIRECT3DDEVICE9 pDev = g_client->GetDevice();
	if (!hPoly) return new vkPolyLine(pDev, pt, npt, (flags&PF_CONNECT) != 0);
	((vkPolyLine *)hPoly)->Update(pt, npt, (flags&PF_CONNECT) != 0);
	return hPoly;
}
...
{
	LPDIRECT3DDEVICE9 pDev = g_client->GetDevice();
	if (!hPoly) return new vkTriangle(pDev, pt, npt, flags);
	((vkTriangle *)hPoly)->Update(pt, npt);
	return hPoly;
}
...
{
	if (hPoly) {
		((vkPolyBase *)hPoly)->Release();
		delete ((vkPolyBase *)hPoly);
	}
}
...
{
	return DWORD((static_cast<vkPadFont *>(hFont))->GetTextLength(pText, len));
}
...
{
	return DWORD((static_cast<vkPadFont *>(hFont))->GetIndexByPosition(pText, pos, len));
}
...
{
	Scene* pScene = g_client->GetScene();
	vkPick pk = pScene->PickMesh(hMesh, (const FMATRIX4*)pWorld, x, y);
	if (pk.group >= 0) {
		if (pk.dist < pm->dist) {
			pm->pos = pk.pos;
			pm->normal = pk.normal;
...
{
	vkEffect::RenderLines((const FVECTOR3*)pVtx, pIdx, nVtx, nIdx, (const FMATRIX4*)pWorld, color);
}
