...
void Create()
{
	...
	if (Config->EnableMeshDbg) {
		dwCmd = oapiRegisterCustomCmd((char*)"vk Debug Controls", (char*)"This dialog allows to control various debug and development features", OpenDlgClbk, NULL);
	}
	else {
		dwCmd = 0;
	}

	dwGFX = oapiRegisterCustomCmd((char*)"vk Graphics Controls", (char*)"This dialog allows to control various graphics options", OpenGFXDlgClbk, NULL);
...
void OpenDlgClbk(void *context)
{
	char buf[64];
	DWORD idx = 0;
	HWND l_hDlg = oapiOpenDialog(g_hInst, IDD_vkMESHDEBUG, WndProc);
...
void UpdateMeshMaterial(float value, DWORD MatPrp, DWORD clr)
{
	...
	switch(MatPrp) {
		...
		case 9:	// SpecialFX
		{
			Mat.ModFlags |= vkMATEX_SPECIALFX;
...
{
	switch (MatPrp) {
		case 0:	return vkMATEX_DIFFUSE;
		case 1:	return vkMATEX_AMBIENT;
		case 2:	return vkMATEX_SPECULAR;
		case 3:	return vkMATEX_EMISSIVE;
		case 4:	return vkMATEX_REFLECT;
		case 5:	return vkMATEX_ROUGHNESS;
		case 6:	return vkMATEX_FRESNEL;
		case 7:	return vkMATEX_EMISSION2;
		case 8:	return vkMATEX_METALNESS;
		case 9:	return vkMATEX_SPECIALFX;
	}
	return 0;
}
...
float GetMaterialValue(DWORD MatPrp, DWORD clr)
{
	...
	const vkMatExt *pMat = hSelMesh->GetMaterial(matidx);	
	if (!pMat) return 0.0f;
...
// =============================================================================================
//
void ValidateMesh(vkMesh* pMesh)
{
	if (pMesh != hSelMesh)
	{	
	...
	case WM_COMMAND:
		...
		switch (LOWORD(wParam)) {
			...
			case IDC_DBG_EXECUTE:
				bPaused = oapiGetPause();
				oapiSetPause(true);
				if (Execute(hWnd, &OpenTex)==false) MessageBox(hWnd,"Failed :(","vk Controls", MB_OK);
				oapiSetPause(bPaused);
				break;
			...
			case IDC_DBG_RELOADSHD:
				vkEffect::vkTechInit(g_client, g_client->GetDevice(), "vkClient");
				break;
