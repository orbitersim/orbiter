...
// Initialization
//
void OapiExtension::GlobalInit(const vkConfig &Config)
{
}
...
// ===========================================================================
// Logs loaded vk DLLs and their versions to Orbiter.log
//
void OapiExtension::LogvkModules(void)
{
	HMODULE hMods[1024];
	HANDLE hProcess;
	DWORD cbNeeded;
	...
	if (EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded))
	{
		for (unsigned int i = 0, n = 0; i < (cbNeeded / sizeof(HMODULE)); ++i)
		{
			...
			if (GetModuleBaseName(hProcess, hMods[i], szModName, ARRAYSIZE(szModName)))
			{
				...
				// Module of interest?
				if (name == "vk.DLL" || 0 == name.compare(0, 6, "D3DX9_"))
				{
