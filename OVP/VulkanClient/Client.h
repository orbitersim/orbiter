...
#define MAX_MESH_LIGHTS		8	// Must match the setting in vkClient.fx
...
struct _vkStats {
	...
	struct {
		vkTime Update;		///< clbkUpdate
		vkTime Scene;			///< clbkRenderScene
		vkTime Display;		///< clbkDisplayFrame
		vkTime FrameTotal;	///< Total frame time
		//------------------------------------------------------------
		vkTime HUDOverlay;	///< Total time spend in HUD, 2D Panel overlay
		vkTime CamVis;		///< Object/camera updates
		vkTime Surface;		///< Surface
		vkTime Clouds;		///< Clouds
		//-------------------------------------------------------------
		vkTime LockWait;		///< Time waiting GetDC or vertex buffer lock
		vkTime BlitTime;		///<
		vkTime GetDC;			///<
...
struct PickProp {
	vkMesh* pMesh;	// Mesh to pick, or NULL for full scene
...
public:
	 ...
	 */
	explicit vkClient (HINSTANCE hInstance);
	...
	*/
	...
	void				PushSketchpad(SURFHANDLE surf, vkPad* pSkp) const;
	...
	class vkPad *		GetTopInterface() const;
