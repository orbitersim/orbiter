...
public:
	 ...
	 */
	static void GlobalInit (oapi::vkClient *gclient);
	 ...
	 */
	virtual vkMesh * GetMesh (UINT idx) { return NULL; }
	 ...
	 */
	virtual void RenderVectors (LPDIRECT3DDEVICE9 dev, vkPad* pSkp);
...
protected:
	...
	void RenderAxisVector(vkPad *pSkp, const FVECTOR4 *pColor, VECTOR3 vector, float lscale, float size, bool bLog=false);
	void RenderAxisLabel(vkPad *pSkp, const FVECTOR4 *clr, VECTOR3 vector, float lscale, float size, const char *label, bool bLog=false);


	static oapi::vkClient *gc;			// graphics client instance pointer
	static SurfNative * blobtex[3];	// beacon textures
	static vkMesh * hStockMesh[16];

	vkSun			sunLight;	// Local copy of sun light. (Can be freely edited)
	bool			bBSRecompute;
