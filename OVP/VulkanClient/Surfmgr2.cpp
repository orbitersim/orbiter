...
void SurfTile::Render ()
{
	...
	const vkClient *pClient = mgr->GetClient();
	...
	if (cfg != PLT_GIANT)
	{
		...
		{
			const vkLight* pLights = scene->GetLights();
	...
	if (DebugControls::IsActive()) {
		...
		if (flags&DBG_FLAGS_TILEBOXES) {
			vkEffect::RenderTileBoundingBox(&mWorld, mesh->Box, &FVECTOR4(1,0,0,1));
		}
	}*/
}
...
void SurfTile::RenderLabels(vkPad *skp, oapi::Font **labelfont, int *fontidx)
{
	if (!label) return;
...
void TileManager2<SurfTile>::Render (MATRIX4 &dwmat, bool use_zbuf, const vPlanet::RenderPrm &rprm)
{
	...
	/*if (scene->GetRenderPass() == RENDERPASS_MAINSCENE) {
		if (GetHandle() == scene->GetCameraNearBody()) {
			vkDebugLog("Body=%s, Elv=%d, Sph=%d", CbodyName(), prevstat.Elev, prevstat.Sphe);
			if (ElevMode == eElevMode::Elevated) vkDebugLog("Elevated");
			if (ElevMode == eElevMode::Spherical) vkDebugLog("Spherical");
			if (ElevMode == eElevMode::ForcedElevated) vkDebugLog("ForcedElevated");
		}
	}*/
...
template<>
void TileManager2<SurfTile>::RenderLabels(vkPad *skp, oapi::Font **labelfont, int *fontidx)
{
	for (int i = 0; i < 2; ++i) {
