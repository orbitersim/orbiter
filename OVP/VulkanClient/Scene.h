...
class Scene {
	...
	friend class vkCelestialSphere;
...
public:
	...
	static void vkTechInit(LPDIRECT3DDEVICE9 pDev, const char *folder);
	...
	Scene (oapi::vkClient *_gc, DWORD w, DWORD h);
	~Scene ();

	/**
	 * \brief Get a pointer to the client
	 */
	//inline const oapi::vkClient *GetClient() const { return gc; }
	...
	const vkSun *GetSun() const { return &sunLight; }
	const vkLight *GetLight(int index) const;
	const vkLight *GetLights() const { return Lights; }
	DWORD GetLightCount() const { return nLights; }
	vkPad* GetPooledSketchpad(int id);
	...
	*/
	...
	void AddParticleStream (class vkParticleStream *_pstream);
	...
	vkPick		PickScene(short xpos, short ypos, const PickProp* p);
	TILEPICK		PickSurface(short xpos, short ypos);
	vkPick		PickMesh(DEVMESHHANDLE hMesh, const FMATRIX4* pW, short xpos, short ypos);
...
private:
	...
	void		RenderVesselMarker(vVessel *vV, vkPad *pSketch);
	float		GetLODLevel(SMapInput* smi);
	void		CombineSMaps(SMapInput* a, SMapInput* b, SMapInput* out);

	LPDIRECT3DTEXTURE9 RenderObjectsInShadow(SMapInput* smi, list<vVessel*>& rList, vkPad *pSkp = nullptr);
	...
	vkCelestialSphere* m_celSphere = {}; // celestial sphere background
	...
	vkParticleStream **pstream = {}; // list of particle streams
