...
// ===========================================================================================
//
MatMgr::MatMgr(class vObject *v, class vkClient *_gc)
{
	gc = _gc;
	vObj = v;
...
// ===========================================================================================
//
void MatMgr::RegisterMaterialChange(vkMesh *pMesh, DWORD midx, const vkMatExt *pM)
{
	if (!pMesh || !pM) return;
...
// ===========================================================================================
//
void MatMgr::RegisterShaderChange(vkMesh *pMesh, WORD id)
{
	if (!pMesh) return;
...
// ===========================================================================================
//
void MatMgr::ApplyConfiguration(vkMesh *pMesh)
{
	...
	if (MeshConfig.count(name)) 
	{
		...
		for (auto x : MeshConfig[name].material) 
		{
			...
			if (flags&vkMATEX_AMBIENT) Mat.Ambient = RecMat.Ambient;
			if (flags&vkMATEX_DIFFUSE) Mat.Diffuse = RecMat.Diffuse;
			if (flags&vkMATEX_EMISSIVE) Mat.Emissive = RecMat.Emissive;
			if (flags&vkMATEX_REFLECT) Mat.Reflect = RecMat.Reflect;
			if (flags&vkMATEX_SPECULAR) Mat.Specular = RecMat.Specular;
			if (flags&vkMATEX_FRESNEL) Mat.Fresnel = RecMat.Fresnel;
			if (flags&vkMATEX_EMISSION2) Mat.Emission2 = RecMat.Emission2;
			if (flags&vkMATEX_ROUGHNESS) Mat.Roughness = RecMat.Roughness;
			if (flags&vkMATEX_METALNESS) Mat.Metalness = RecMat.Metalness;
...
bool MatMgr::LoadConfiguration(bool bAppend)
{
	...
	{	
		...
		if (!strncmp(cbuf, "EMISSION2", 9)) {
			...
			Mat.ModFlags |= vkMATEX_EMISSION2;
			continue;
		}
		...
		if (!strncmp(cbuf, "ROUGHNESS", 9)) {
			...
			Mat.ModFlags |= vkMATEX_ROUGHNESS;
			continue;
		}
		...
		if (!strncmp(cbuf, "METALNESS", 9)) {
			...
			Mat.Metalness = a;
			Mat.ModFlags |= vkMATEX_METALNESS;
			continue;
		}
	}
...
bool MatMgr::SaveConfiguration()
{
	...
	{		
		...
		{		
			...
			if (flags&vkMATEX_AMBIENT)  fprintf(file.pFile,"AMBIENT %f %f %f\n", pM->Ambient.x, pM->Ambient.y, pM->Ambient.z);
			if (flags&vkMATEX_DIFFUSE)  fprintf(file.pFile,"DIFFUSE %f %f %f %f\n", pM->Diffuse.x, pM->Diffuse.y, pM->Diffuse.z, pM->Diffuse.w);
			if (flags&vkMATEX_SPECULAR) fprintf(file.pFile,"SPECULAR %f %f %f %f\n", pM->Specular.x, pM->Specular.y, pM->Specular.z, pM->Specular.w);
			if (flags&vkMATEX_EMISSIVE) fprintf(file.pFile,"EMISSIVE %f %f %f\n", pM->Emissive.x, pM->Emissive.y, pM->Emissive.z);
			if (flags&vkMATEX_REFLECT)  fprintf(file.pFile,"REFLECT %f %f %f\n", pM->Reflect.x, pM->Reflect.y, pM->Reflect.z);
			if (flags&vkMATEX_FRESNEL)  fprintf(file.pFile,"FRESNEL %f %f %f\n", pM->Fresnel.x, pM->Fresnel.z, pM->Fresnel.y);
			if (flags&vkMATEX_EMISSION2) fprintf(file.pFile, "EMISSION2 %f %f %f\n", pM->Emission2.x, pM->Emission2.y, pM->Emission2.z);
			if (flags&vkMATEX_ROUGHNESS) fprintf(file.pFile, "SMOOTHNESS %f %f\n", pM->Roughness.x, pM->Roughness.y);
			if (flags&vkMATEX_METALNESS) fprintf(file.pFile, "METALNESS %f\n", pM->Metalness);		
		}
	}
	return true;
}
