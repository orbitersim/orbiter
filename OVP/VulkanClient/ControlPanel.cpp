...
void vkClient::Label(const char *format, ...)
{
	char buffer[256];
	va_list args;
...
void vkClient::DrawTimeBar(double t, double s, double f, DWORD color, const char *label)
{
	static double x = 8;
	static int z = 100;
	char legend[256];
...
void vkClient::RenderControlPanel()
{
	...
	pItemsSkp->Text(20,70,"vkClient Statistics",21);
	...
	for (auto& x : vkStats.TilesRendered) {
		if (x.second != 0) TileBuf[x.first] = x.second;
		x.second = 0;
	}

	if (vkStats.TilesRendered.size()) {
	...
	Label("Tiles Allocated......: %u", vkStats.TilesAllocated);
...
//	static double prt_blr;
	...
	if (oapiGetSysMJD() > systime) {
		...
		frames = vkStats.Timer.Scene.count;
		double iframes = 1.0 / frames;

		matchg = DWORD(double(vkStats.Mesh.MtrlChanges) * iframes);
		texchg = DWORD(double(vkStats.Mesh.TexChanges) * iframes);
		verts = DWORD(double(vkStats.Mesh.Vertices) * iframes);
		grps = DWORD(double(vkStats.Mesh.MeshGrps) * iframes);
		meshes = DWORD(double(vkStats.Mesh.Meshes) * iframes);
		DCPeak = vkStats.Timer.GetDC.peak;
		LockPeak = vkStats.Timer.LockWait.peak;

		double total = vkStats.Timer.FrameTotal.time;

		scene   = vkStats.Timer.Scene.time;
		update  = vkStats.Timer.Update.time;
		display = vkStats.Timer.Display.time;
		lock    = vkStats.Timer.LockWait.time;
		blit    = vkStats.Timer.BlitTime.time;
		getdc	= vkStats.Timer.GetDC.time;
		...
		// -------------------------------------
		Reset(vkStats.Timer.CamVis);
		Reset(vkStats.Timer.Scene);
		Reset(vkStats.Timer.Update);
		Reset(vkStats.Timer.Display);
		Reset(vkStats.Timer.LockWait);
		Reset(vkStats.Timer.BlitTime);
		Reset(vkStats.Timer.GetDC);
		Reset(vkStats.Timer.FrameTotal);
		// -------------------------------------
		Reset(vkStats.Timer.HUDOverlay);
		// -------------------------------------
		memset(&vkStats.Mesh, 0, sizeof(vkStats.Mesh));
	}
...
bool vkClient::ControlPanelMsg(WPARAM wParam)
{
	return false;
}
