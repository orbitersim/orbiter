...
public:
	CloudManager (oapi::vkClient *gclient, const vPlanet *vplanet);

	void Render(LPDIRECT3DDEVICE9 dev, FMATRIX4 &wmat, double scale, int level, double viewap = 0.0);
	void RenderShadow(LPDIRECT3DDEVICE9 dev, FMATRIX4 &wmat, double scale, int level, double viewap, float shadowalpha);
	void LoadData();
