...
//				 2010 - 2022 Jarmo Nikkanen (vkClient implementation)
// ==============================================================
...
LPDIRECT3DTEXTURE9 vkMesh::pShadowMap[SHM_CASCADE_COUNT] = {};
FVECTOR4 vkMesh::ShdSubRect[SHM_CASCADE_COUNT] = {};
MeshShader* vkMesh::s_pShader[16] = {};
...
MeshBuffer::MeshBuffer(DWORD _nVtx, DWORD _nFace, const class vkMesh *_pRoot)
{
	nVtx = _nVtx;
	nIdx = _nFace * 3;
	nRef = 1;
...
MeshBuffer::MeshBuffer(MeshBuffer *pSrc, const class vkMesh *_pRoot)
{
	nVtx = pSrc->nVtx;
	nIdx = pSrc->nIdx;
	nRef = 1;
	...
	memcpy(pGBSys, pSrc->pGBSys, sizeof(XMVECTOR) * nVtx);
...
void MeshBuffer::Map(LPDIRECT3DDEVICE9 pDev)
{
	...
	if (!pGB) {
		HR(pDev->CreateVertexBuffer(nVtx * sizeof(FVECTOR4), Usage, 0, D3DPOOL_DEFAULT, &pGB, NULL));
	}
	if (!pIB) {
	...
	memcpy(pTgt, pGBSys, nVtx * sizeof(FVECTOR4));
	HR(pGB->Unlock());
...
// ======================================================================================
//
void vkMesh::Null(const char *meshName /* = NULL */)
{
	nGrp = 0;
	Grp = NULL;
	nTex = 0;
...
// ===========================================================================================
//
vkMesh::vkMesh(const char *fname) : vkEffect()
{
	Null();
...
// ===========================================================================================
//
vkMesh::vkMesh(MESHHANDLE hMesh, bool asTemplate, FVECTOR3 *reorig, float *scale) : vkEffect()
{
	Null();
...
// ===========================================================================================
//
vkMesh::vkMesh(DWORD groups, const MESHGROUPEX **hGroup, const SURFHANDLE *hSurf) : vkEffect()
{
	Null();
	nGrp = groups;
	...
	oapiMatrixIdentity(&mTransform);
	oapiMatrixIdentity(&mTransformInv);
	MeshCatalog.insert(this);
...
// ===========================================================================================
//
vkMesh::vkMesh(const MESHGROUPEX *pGroup, const MATERIAL *pMat, SurfNative *pTex) : vkEffect()
{
	Null();
...
//
vkMesh::vkMesh(MESHHANDLE hMesh, const vkMesh &hTemp)
{
	...
	if (nMtrl) Mtrl = new vkMatExt[nMtrl];
...
void vkMesh::ReLoadMeshFromHandle(MESHHANDLE hMesh)
{
	hOapiMesh = hMesh;
...
// ===========================================================================================
//
const char* vkMesh::GetDirName(int i, int v)
{
...
// ===========================================================================================
//
void vkMesh::BakeLights(ImageProcessing* pBaker, const FVECTOR3* BakedLightsControl)
{
...
// ===========================================================================================
//
void vkMesh::BakeAO(ImageProcessing* pBaker, const FVECTOR3 &vSun, const LVLH &lvlh, const LPDIRECT3DTEXTURE9 pIrrad)
{
	if (!pBaker->IsOK()) return; // Baker not initialized
	if (DefShader != SHADER_BAKED_VC) return; // Not supported by shader
	if (BakedLights.size() == 0) return; // Nothing to bake
	...
	for (int i = 0; i < 6; i++) {
		...
		float z = dotp(lvlh.Up, DirL);	// lvlh is in a local vessel frame
		FVECTOR2 p = FVECTOR2(dotp(lvlh.East, DirL), dotp(lvlh.North, DirL)) / (1.0f + abs(z));
	...
	for (int i = 0; i < 6; i++)
	{
		...
		if (tex)
		{
			...
			control[i] = saturate(dotp(GetDir(i), vSun));
			control[i] *= control[i];
		}	
	}
...
// ===========================================================================================
//
LPDIRECT3DTEXTURE9 vkMesh::GetCombinedMap(int tex_idx)
{
...
// ===========================================================================================
//
void vkMesh::LoadMeshFromHandle(MESHHANDLE hMesh, FVECTOR3 *reorig, float *scale)
{
	hOapiMesh = hMesh;
	const char* meshn = oapiGetMeshFilename(hMesh);
	strcpy_s(name, 128, meshn ? meshn : "???");
...
// ===========================================================================================
//
void vkMesh::SetName(const char *name_)
{
...
void vkMesh::ProcessInherit()
{
	...
	if (bPopUp) MessageBoxA(NULL, "Invalid Mesh Detected", "vkClient Error:",MB_OK);
}


// ===========================================================================================
//
FVECTOR3 vkMesh::GetGroupSize(DWORD idx) const
{
	if (!IsOK()) return FVECTOR3(0,0,0);
	if (idx>=nGrp) return FVECTOR3(0,0,0);
	if (Grp[idx].nVert<2) return FVECTOR3(0,0,0);
	return _F(Grp[idx].BBox.mx - Grp[idx].BBox.mn);
}


// ===========================================================================================
//
void vkMesh::ResetTransformations()
{
	_TRACE;
	if (!IsOK()) return;
	oapiMatrixIdentity(&mTransform);
	...
	bBSRecomputeAll = true;
	for (DWORD i=0;i<nGrp;i++) {
		oapiMatrixIdentity(&Grp[i].Transform);
		oapiMatrixIdentity(&pGrpTF[i]);
...
// ===========================================================================================
//
void vkMesh::UpdateTangentSpace(NMVERTEX *pVrt, WORD *pIdx, DWORD nVtx, DWORD nFace, bool bTextured)
{
	if (!IsOK()) return;
...
// ===========================================================================================
//
void vkMesh::SetGroupRec(DWORD i, const MESHGROUPEX *mg)
{
	if (i>=nGrp) return;
...
// ===========================================================================================
//
bool vkMesh::CopyVertices(GROUPREC *grp, const MESHGROUPEX *mg, FVECTOR3 *reorig, float *scale)
{
	if (!pBuf) return false;
	NTVERTEX *pNT = mg->Vtx;
	SMVERTEX *pShad = pBuf->pSBSys + grp->VertOff;
	NMVERTEX *pVert = pBuf->pVBSys + grp->VertOff;
	XMVECTOR *pGeo  = pBuf->pGBSys + grp->VertOff;
	...
	for (DWORD i=0;i<mg->nVtx; i++) {
		...
		pGeo[i] = FVECTOR4(pVert[i].x, pVert[i].y, pVert[i].z, 0.0f).XM();
...
//
int vkMesh::EditGroup(DWORD grp, GROUPEDITSPEC *ges)
{
	_TRACE;
	if (!IsOK()) return 1;
	...
	if (flag & GRPEDIT_VTX) {
		...
		XMVECTOR *pGeo = pBuf->pGBSys + g->VertOff;
			...
			for (i = 0; i < ges->nVtx; i++) {
				...
				if (vi < g->nVert) {
					...
					if ((flag & GRPEDIT_VTXCRD)!=0 || (flag & GRPEDIT_VTXCRDADD)!=0) {
						pGeo[vi] = FVECTOR4(vtx[vi].x, vtx[vi].y, vtx[vi].z, 0.0f).XM();
...
int vkMesh::GetGroup (DWORD grp, GROUPREQUESTSPEC *grs)
{
	if (!pBuf) return 1;
...
// ===========================================================================================
//
void vkMesh::SetMFDScreenId(DWORD idx, WORD id)
{
...
// ===========================================================================================
//
bool vkMesh::SetTexture(DWORD texidx, SURFHANDLE tex)
{
	_TRACE;
	if (!IsOK()) return false;
	if (texidx >= nTex) {
		LogErr("vkMesh::SetTexture(%u, %s) index out of range",texidx, _PTR(tex));
		return false;
	}
	Tex[texidx] = (lpSurfNative)tex;
	LogBlu("vkMesh(%s)::SetTexture(%u, %s) (%s)", _PTR(this), texidx, _PTR(tex), SURFACE(tex)->GetName());
	CheckMeshStatus();
	return true;
}
...
// ===========================================================================================
//
DWORD vkMesh::GetMeshGroupMaterialIdx(DWORD idx) const
{
	if (!IsOK()) return 0;
...
// ===========================================================================================
//
DWORD vkMesh::GetMeshGroupTextureIdx(DWORD idx) const
{
	if (!IsOK()) return 0;
	if (idx>=nGrp) return 0;
...
// ===========================================================================================
//
bool vkMesh::HasTexture(SURFHANDLE hSurf) const
{
	if (!IsOK()) return false;
...
// ===========================================================================================
//
void vkMesh::SetTexMixture(DWORD ntex, float mix)
{
	_TRACE;
	if (!IsOK()) return;
	ntex--;
...
// ===========================================================================================
//
void vkMesh::SetSunLight(const vkSun *light)
{
	memcpy(&sunLight, light, sizeof(vkSun));
}

// ===========================================================================================
//
DWORD vkMesh::GetVertexCount(int grp) const
{
...
// ===========================================================================================
//
DWORD vkMesh::GetIndexCount(int grp) const
{
...
// ===========================================================================================
//
DWORD vkMesh::GetGroupTransformCount() const
{
	DWORD cnt = 0;
...
// ===========================================================================================
//
const vkMesh::GROUPREC *vkMesh::GetGroup(DWORD idx) const
{
	if (!IsOK()) return NULL;
...
// ===========================================================================================
//
vkMesh::GROUPREC* vkMesh::GetGroup(DWORD idx)
{
	if (!IsOK()) return NULL;
	if (idx < nGrp) return &Grp[idx];
	return NULL;
...
// ===========================================================================================
//
const vkMatExt * vkMesh::GetMaterial(DWORD idx) const
{
...
// ===========================================================================================
//
bool vkMesh::GetMaterial(vkMatExt *pMat, DWORD idx) const
{
	if (pMat && idx<nMtrl) {
		memcpy(pMat, &Mtrl[idx], sizeof(vkMatExt));
		return true;
	}
	return false;
}
...
// ===========================================================================================
//
void vkMesh::SetMaterial(const D3DMATERIAL9 *pMat, DWORD idx, bool bStat)
{
	vkMatExt Mat;
...
// ===========================================================================================
//
void vkMesh::SetMaterial(const vkMatExt *pMat, DWORD idx, bool bStat)
{
	if (idx < nMtrl) {
		memcpy(&Mtrl[idx], pMat, sizeof(vkMatExt));
...
//
int vkMesh::SetMaterialEx(DWORD idx, MatProp mid, const FVECTOR4* value)
{
	...
	if (value) {
		...
		case MatProp::Diffuse:
			Mtrl[idx].Diffuse = *((FVECTOR4*)value);
			Mtrl[idx].ModFlags |= vkMATEX_DIFFUSE;
			bMtrlModidied = true;
			return 0;
		case MatProp::Ambient:
			Mtrl[idx].Ambient = *((FVECTOR3*)value);
			Mtrl[idx].ModFlags |= vkMATEX_AMBIENT;
			bMtrlModidied = true;
			return 0;
		case MatProp::Specular:
			Mtrl[idx].Specular = *((FVECTOR4*)value);
			Mtrl[idx].ModFlags |= vkMATEX_SPECULAR;
			bMtrlModidied = true;
			return 0;
		case MatProp::Light:
			Mtrl[idx].Emissive = *((FVECTOR3*)value);
			Mtrl[idx].ModFlags |= vkMATEX_EMISSIVE;
			bMtrlModidied = true;
			return 0;
		case MatProp::Emission:
			Mtrl[idx].Emission2 = *((FVECTOR3*)value);
			Mtrl[idx].ModFlags |= vkMATEX_EMISSION2;
			bMtrlModidied = true;
			return 0;
		case MatProp::Reflect:
			Mtrl[idx].Reflect = *((FVECTOR3*)value);
			Mtrl[idx].ModFlags |= vkMATEX_REFLECT;
			bMtrlModidied = true;
			return 0;
		case MatProp::Smooth:
			Mtrl[idx].Roughness = FVECTOR2(value->g, value->r);
			Mtrl[idx].ModFlags |= vkMATEX_ROUGHNESS;
			bMtrlModidied = true;
			return 0;
		case MatProp::Fresnel:
			Mtrl[idx].Fresnel = *((FVECTOR3*)value);
			Mtrl[idx].ModFlags |= vkMATEX_FRESNEL;
			bMtrlModidied = true;
			return 0;
		case MatProp::Metal:
			Mtrl[idx].Metalness = value->r;
			Mtrl[idx].ModFlags |= vkMATEX_METALNESS;
			bMtrlModidied = true;
			return 0;
		case MatProp::SpecialFX:
			Mtrl[idx].SpecialFX = *((FVECTOR4*)value);
			Mtrl[idx].ModFlags |= vkMATEX_SPECIALFX;
			bMtrlModidied = true;
			return 0;
		}
		return 5;
	...
	if (value == NULL)
	{
		switch (mid) {
		case MatProp::Diffuse:
			Mtrl[idx].Diffuse = FVECTOR4(1, 1, 1, 1);
			Mtrl[idx].ModFlags &= (~vkMATEX_DIFFUSE);
			bMtrlModidied = true;
			return 0;
		case MatProp::Ambient:
			Mtrl[idx].Ambient = FVECTOR3(1, 1, 1);
			Mtrl[idx].ModFlags &= (~vkMATEX_AMBIENT);
			bMtrlModidied = true;
			return 0;
		case MatProp::Specular:
			Mtrl[idx].Specular = FVECTOR4(1, 1, 1, 20);
			Mtrl[idx].ModFlags &= (~vkMATEX_SPECULAR);
			bMtrlModidied = true;
			return 0;
		case MatProp::Light:
			Mtrl[idx].Emissive = FVECTOR3(0, 0, 0);
			Mtrl[idx].ModFlags &= (~vkMATEX_EMISSIVE);
			bMtrlModidied = true;
			return 0;
		case MatProp::Emission:
			Mtrl[idx].Emission2 = FVECTOR3(1, 1, 1);
			Mtrl[idx].ModFlags &= (~vkMATEX_EMISSION2);
			bMtrlModidied = true;
			return 0;
		case MatProp::Reflect:
			Mtrl[idx].Reflect = FVECTOR3(0.0f, 0, 0);
			Mtrl[idx].ModFlags &= (~vkMATEX_REFLECT);
			bMtrlModidied = true;
			return 0;
		case MatProp::Smooth:
			...
			return 0;
		case MatProp::Fresnel:
			Mtrl[idx].Fresnel = FVECTOR3(1, 0, 1024.0f);
			Mtrl[idx].ModFlags &= (~vkMATEX_FRESNEL);
			bMtrlModidied = true;
			return 0;
		case MatProp::Metal:
			...
			return 0;
		case MatProp::SpecialFX:
			Mtrl[idx].SpecialFX = FVECTOR4(0.0f, 0, 0, 0);
			Mtrl[idx].ModFlags &= (~vkMATEX_SPECIALFX);
			bMtrlModidied = true;
			return 0;
		}
...
//4 = material index out of range, 5 = material property not supported by shader used by the mesh.
//
int vkMesh::GetMaterialEx(DWORD idx, MatProp mid, FVECTOR4* value)
{
	if (idx >= nMtrl) return 4;
	...
	if (value)
	{
		...
		{
		case MatProp::Diffuse:
			*((FVECTOR4*)value) = Mtrl[idx].Diffuse;
			return 0;
		case MatProp::Ambient:
			*((FVECTOR3*)value) = Mtrl[idx].Ambient;
			return 0;
		case MatProp::Specular:
			*((FVECTOR4*)value) = Mtrl[idx].Specular;
			return 0;
		case MatProp::Light:
			*((FVECTOR3*)value) = Mtrl[idx].Emissive;
			return 0;
		case  MatProp::Emission:
			if ((Mtrl[idx].ModFlags&vkMATEX_EMISSION2) == 0) return -2;
			*((FVECTOR3*)value) = Mtrl[idx].Emission2;
			return 0;
		case MatProp::Reflect:
			if ((Mtrl[idx].ModFlags&vkMATEX_REFLECT) == 0) return -2;
			*((FVECTOR3*)value) = Mtrl[idx].Reflect;
			return 0;
		case MatProp::Smooth:
			if ((Mtrl[idx].ModFlags&vkMATEX_ROUGHNESS) == 0) return -2;
			value->g = Mtrl[idx].Roughness.x;
			value->r = Mtrl[idx].Roughness.y;
			return 0;
		case MatProp::Fresnel:
			if ((Mtrl[idx].ModFlags&vkMATEX_FRESNEL) == 0) return -2;
			*((FVECTOR3*)value) = Mtrl[idx].Fresnel;
			return 0;
		case MatProp::Metal:
			if ((Mtrl[idx].ModFlags& vkMATEX_METALNESS) == 0) return -2;
			value->r = Mtrl[idx].Metalness;
			return 0;
		case MatProp::SpecialFX:
			if ((Mtrl[idx].ModFlags& vkMATEX_SPECIALFX) == 0) return -2;
			*((FVECTOR4*)value) = Mtrl[idx].SpecialFX;
			return 0;
		}
		return 5;
	}
...
// ===========================================================================================
//
void vkMesh::SetAmbientColor(const FVECTOR3& c)
{
	_TRACE;
	if (!IsOK()) return;
	cAmbient = c;
...
// ===========================================================================================
//
const FVECTOR3& vkMesh::GetAmbientColor()
{
	_TRACE;
	return cAmbient;
}
...
// ===========================================================================================
//
void vkMesh::SetupFog(const FMATRIX4* pW)
{
	_TRACE;
	if (!IsOK()) return;
	FX->SetVector(eAttennuate, _DX(FVECTOR4(1.0f, 1, 1, 1)));
	FX->SetVector(eInScatter,  _DX(FVECTOR4(0.0f, 0, 0, 0)));
}

// ===========================================================================================
//
void vkMesh::RenderGroup(int idx)
{
...
// ===========================================================================================
//
void vkMesh::RenderGroup(const GROUPREC *grp)
{
	_TRACE;
	if (!IsOK()) return;
	if (!grp) return;
	...
	vkStats.Mesh.Vertices += grp->nVert;
	vkStats.Mesh.MeshGrps++;
}
...
// ===========================================================================================
//
bool vkMesh::IsGroupRendered(DWORD idx) const
{
...
void vkMesh::CheckMeshStatus()
{
	...
	for (DWORD g = 0; g < nGrp; g++) {
		...
		if (mat->ModFlags&vkMATEX_SPECULAR) Grp[g].PBRStatus |= 0x1;
		if (mat->ModFlags&vkMATEX_REFLECT) Grp[g].PBRStatus |= 0x8;
		if (mat->ModFlags&vkMATEX_ROUGHNESS) Grp[g].PBRStatus |= 0x4;
		if (mat->ModFlags&vkMATEX_FRESNEL) Grp[g].PBRStatus |= 0x10;
	}
...
// ===========================================================================================
//
void vkMesh::SetDefaultShader(WORD shader)
{
	DefShader = shader;
	bMtrlModidied = true;
	CheckMeshStatus();
...
void vkMesh::ConfigureAtmo()
{
	...
	FX->SetValue(eSun, &sunLight, sizeof(vkSun));
}
...
void vkMesh::ConfigureShadows()
{
	...
	HR(FX->SetValue(vkEffect::eSHDSubRect, ShdSubRect, sizeof(ShdSubRect)));
}


// ===========================================================================================
// static:
void vkMesh::SetShadows(const SHADOWMAP *sprm)
{
	if (sprm) {
		for (int i = 0; i < SHM_CASCADE_COUNT; i++) {
...
//
void vkMesh::Render(const FMATRIX4* pW, const ENVCAMREC* em, int iTech)
{
	_TRACE;
	...
	HR(vkEffect::FX->SetBool(vkEffect::eBaseBuilding, bShadowProjection));
	HR(vkEffect::FX->SetBool(vkEffect::eCockpit, bVirtualCockpit));

	FVECTOR4 Field;
	FMATRIX4 mWorldView,  q;

	oapiMatrixMultiply(&mWorldView, pW, scn->GetViewMatrix());
	...
	if (bGlobalTF) oapiMatrixMultiply(&mWorldMesh, &mTransform, pW);
	else mWorldMesh = *pW;

	vkStats.Mesh.Meshes++;

	vkMatExt *mat, *old_mat = NULL;
	...
	FX->SetVector(eColor, _DX(FVECTOR4(0.0f, 0, 0, 0)));
	...
	// Setup Local lights -------------------------------------------
	//
	const vkLight *pLights = gc->GetScene()->GetLights();
	...
	if (pLights && nSceneLights>0) {
		...
		FVECTOR3 pos = oapiTransformCoord(&_F(BBox.bs), pW);
	...
	for (DWORD g=0; g<nGrp; g++) {
		...
		if (DebugControls::IsActive()) {
			...
			if (bActiveVisual) {
				...
				FX->SetVector(eColor, _DX(_F4(0, 0, 0, 0)));

				if (flags&DBG_FLAGS_HLMESH) {
					if (g_uCurrentMesh==selmsh) {
						FX->SetVector(eColor, _DX(FVECTOR4(0.0f, 0.0f, 0.5f, 0.5f)));
					}
				}
				if (flags&DBG_FLAGS_HLGROUP) {				
					if (g == selgrp && g_uCurrentMesh == selmsh) {
						FX->SetVector(eColor, _DX(FVECTOR4(0.0f, 0.5f, 0.0f, 0.5f)));
					}
				}
			}
		}
		...
		if (Grp[g].MFDScreenId) {
			...
			FX->SetValue(eMtrl, mat, sizeof(vkMatExt)-4);
		}
		...
		if (Grp[g].bTransform) {
			bWorldMesh = false;
			FX->SetMatrix(eW, _DX(oapiMatrixMultiply(&q, &pGrpTF[g], pW)));
		}
		else if (!bWorldMesh) {
			FX->SetMatrix(eW, _DX(mWorldMesh));
			bWorldMesh = true;
		}
		...
		if (bNoC) FX->SetValue(eNoColor, &FVECTOR3(1, 1, 1), sizeof(FVECTOR3));
		else FX->SetValue(eNoColor, &FVECTOR3(0, 0, 0), sizeof(FVECTOR3));
		...
		vkStats.Mesh.Vertices += Grp[g].nVert;
		vkStats.Mesh.MeshGrps++;
...
//
void vkMesh::RenderSimplified(const FMATRIX4* pW, LPDIRECT3DCUBETEXTURE9 *pEnv, int nEnv, bool bSP)
{
	if (!IsOK()) return;
	...
	HR(vkEffect::FX->SetBool(vkEffect::eBaseBuilding, bSP));
	...
	//vkDebugLog("Mesh=[%s], nLights=%d", GetName(), nSceneLights);
...
//
void vkMesh::RenderFast(const FMATRIX4* pW, int iTech)
{
	...
	if (pLights && nSceneLights>0) {
		...
		int nMeshLights = 0;
		FVECTOR3 pos = oapiTransformCoord(ptr(_F(BBox.bs)), pW);
	...
	for (DWORD g = 0; g<nGrp; g++) {
		...
		if ((Grp[g].UsrFlag & 0x10) != 0) FX->SetValue(eNoColor, &FVECTOR3(1, 1, 1), sizeof(FVECTOR3));
		else FX->SetValue(eNoColor, &FVECTOR3(0, 0, 0), sizeof(FVECTOR3));
...
// ===========================================================================================
//
FMATRIX4 vkMesh::GetTransform(int g, bool bCombined)
{
	...
	FMATRIX4 Ident; oapiMatrixIdentity(&Ident);
	return Ident;
}
...
// ===========================================================================================
//
bool vkMesh::SetTransform(int g, const FMATRIX4* pMat)
{
	...
	if (g < 0) {
		...
		for (DWORD i = 0; i<nGrp; i++) {
			if (Grp[i].bTransform) oapiMatrixMultiply(&pGrpTF[i], &mTransform, &Grp[i].Transform);
	...
	oapiMatrixMultiply(&pGrpTF[g], &mTransform, &Grp[g].Transform);
...
// ===========================================================================================
//
void vkMesh::RenderBaseTile(const FMATRIX4* pW)
{
	if (!IsOK()) return;
	...
	FX->SetVector(eColor, _DX(F4_Zero));
	FX->SetMatrix(eGT, _DX(FMATRIX_Identity));
	FX->SetMatrix(eW, _DX(pW));
...
// ================================================================================================
//
void vkMesh::RenderShadowMap(const FMATRIX4* pW, const FMATRIX4* pVP, int opt, bool bNoCull)
{
	if (!IsOK()) return;

	pBuf->Map(pDev);

	FMATRIX4 GroupMatrix, mWorldMesh;

	MeshShader* pShader = nullptr;
	
	MeshShader::vs_const.mVP = pVP ? *pVP : FMATRIX4();

	oapiMatrixIdentity(&MeshShader::vs_const.mW);
	...
	if (opt == 1) {
		...
		if (Flags & 0x20) {
			...
			pDev->SetStreamSource(0, pBuf->pGB, 0, sizeof(FVECTOR4));
	...
	for (DWORD g = 0; g < nGrp; g++)
	{
		...
		if (Grp[g].bTransform) {
			oapiMatrixMultiply(&MeshShader::vs_const.mW, &pGrpTF[g], pW);		// Apply Animations to instance matrices
			bInit = true;
		}
		else {
...
// ================================================================================================
//
void vkMesh::RenderStencilShadows(float alpha, const FMATRIX4* pP, const FMATRIX4* pW, bool bShadowMap, const FVECTOR4 *elev)
{
	if (!IsOK()) return;
	...
	if (elev) FX->SetVector(eInScatter, _DX(elev));
	else FX->SetVector(eInScatter, _DX(FVECTOR4(0.0f, 1, 0, 0)));
	...
	for (DWORD g = 0; g < nGrp; g++) {
		...
		if (Grp[g].bTransform) {
			oapiMatrixMultiply(&GroupMatrix, &pGrpTF[g], pW);		// Apply Animations to instance matrices
...
// ================================================================================================
//
void vkMesh::RenderShadowsEx(float alpha, const FMATRIX4* pP, const FMATRIX4* pW, const FVECTOR4 *light, const FVECTOR4 *param)
{
	if (!IsOK()) return;
	...
	FX->SetTechnique(eShadowTech);
	FX->SetMatrix(eW, _DX(pW));
	FX->SetMatrix(eGT, _DX(pP));
	FX->SetFloat(eMix, alpha);
	if (light) FX->SetVector(eColor, _DX(light));
	else FX->SetVector(eColor, _DX(FVECTOR4(0.0f, 1, 0, 0)));
	FX->SetVector(eTexOff, _DX(param));
...
// This is a rendering routine for a Exterior Mesh, non-spherical moons/asteroids
//
void vkMesh::RenderBoundingBox(const FMATRIX4* pW)
{
	_TRACE;
	...
	if (flags&DBG_FLAGS_BOXES) {
		...
		// ----------------------------------------------------------------
		FX->SetMatrix(eW, _DX(pW));
		FX->SetVector(eColor, _DX(FVECTOR4(0, 1, 0, 0.5f)));
		...
		for (DWORD g=0; g<nGrp; g++) {
			...
			FX->SetVector(eAttennuate, _DX(Grp[g].BBox.mn));
			FX->SetVector(eInScatter, _DX(Grp[g].BBox.mx));

			// Apply Animations =========================================================================================
			//
			if (Grp[g].bTransform) {
				if (bGlobalTF)  FX->SetMatrix(eGT, _DX(oapiMatrixMultiply(&q, &mTransform, &Grp[g].Transform)));
				else FX->SetMatrix(eGT, _DX(Grp[g].Transform));
			}
			else FX->SetMatrix(eGT, _DX(mTransform));
	...
	if (flags&DBG_FLAGS_SPHERES) {
		for (DWORD g=0; g<nGrp; g++) {
			if (flags&DBG_FLAGS_SELGRPONLY && g!=selgrp) continue;
			if (Grp[g].UsrFlag & 0x2) continue;
			vkEffect::RenderBoundingSphere(pW, NULL, &Grp[g].BBox.bs, ptr(FVECTOR4(0,1,0,0.75f)));
		}
	}
	if (flags&DBG_FLAGS_BOXES) vkEffect::RenderBoundingBox(pW, &mTransform, &BBox.mn, &BBox.mx, ptr(FVECTOR4(0,0,1,0.75f)));
	if (flags&DBG_FLAGS_SPHERES) vkEffect::RenderBoundingSphere(pW, &mTransform, &BBox.bs, ptr(FVECTOR4(0,0,1,0.75f)));
}
...
void vkMesh::BoundingBox(const NMVERTEX *vtx, DWORD n, D9BBox *box)
{
	XMVECTOR mi, mx;
	...
	XMStoreFloat4((XMFLOAT4 *)&box->mn.x, XMVectorSetW(mi, 0));
	XMStoreFloat4((XMFLOAT4 *)&box->mx.x, XMVectorSetW(mx, 0));
}

// ===========================================================================================
//
void vkMesh::TransformGroup(DWORD n, const FMATRIX4 *m)
{
	if (!IsOK()) return;

	bBSRecompute = true;
	oapiMatrixMultiply(&Grp[n].Transform, &Grp[n].Transform, m);

	Grp[n].bTransform = true;
	Grp[n].bUpdate = true;

	oapiMatrixMultiply(&pGrpTF[n], &mTransform, &Grp[n].Transform);
}

// ===========================================================================================
//
void vkMesh::Transform(const FMATRIX4 *m)
{
	if (!IsOK()) return;
	...
	oapiMatrixMultiply(&mTransform, &mTransform, m);
	oapiMatrixInverse(&mTransformInv, NULL, &mTransform);
...
// ===========================================================================================
//
void vkMesh::SetPosition(VECTOR3 &pos)
{
	bGlobalTF = true;
...
// ===========================================================================================
//
void vkMesh::SetRotation(FMATRIX4 &rot)
{
	bGlobalTF = true;
...
void vkMesh::UpdateBoundingBox()
{
	...
	if (nGrp==0) {
		BBox.mn = FVECTOR4(0.0f, 0,0,0);
		BBox.mx = FVECTOR4(0.0f, 0,0,0);
	}
	else {
		for (DWORD i=0;i<nGrp;i++) {
			if (Grp[i].bTransform) {
				if (bGlobalTF) {
					FMATRIX4 q;
					oapiMatrixMultiply(&q, oapiMatrixMultiply(&q, &mTransform, &Grp[i].Transform), &mTransformInv);
...
// ===========================================================================================
//
FVECTOR3 vkMesh::GetBoundingSpherePos()
{
	if (!IsOK()) return FVECTOR3(0,0,0);
	UpdateBoundingBox();
	return _F(BBox.bs);
...
// ===========================================================================================
//
float vkMesh::GetBoundingSphereRadius()
{
	if (!IsOK()) return 0.0f;
	UpdateBoundingBox();
	return BBox.bs.w;
...
// ===========================================================================================
//
vkPick vkMesh::Pick(const FMATRIX4* pW, const FMATRIX4* pT, const FVECTOR3 *vDir, const PickProp *p)
{
	vkPick result;
	result.dist  = 1e30f;
	result.pMesh = NULL;
	...
	if (!pBuf->pGBSys || !pBuf->pIBSys) {
		LogErr("vkMesh::Pick() Failed: No Geometry Available");
		return result;
	}

	UpdateBoundingBox();

	XMVECTOR Zero = FVECTOR3(0).XM();
	XMVECTOR Dir = vDir->XM();

	XMMATRIX mW, mWT, mWorldMesh;

	if (pT) mWT = XMMatrixMultiply(pT->XM(), pW->XM());
	else mWT = pW->XM();

	if (bGlobalTF) mWorldMesh = XMMatrixMultiply(mTransform.XM(), mWT);
	else mWorldMesh = mWT;
	...
	for (DWORD g=0;g<nGrp;g++) {
		...
		FVECTOR3 bs = _F(Grp[g].BBox.bs);
		float rad = Grp[g].BBox.bs.w * D3DMAT_BSScaleFactor(&fmWT);

		bs = oapiTransformCoord(&bs, &fmWT);
		...
		if (Grp[g].bTransform) mW = XMMatrixMultiply(pGrpTF[g].XM(), mWT);
		else mW = mWorldMesh;

		XMVECTOR _a, _b, _c, cp;

		WORD *pIdc = pBuf->pIBSys + Grp[g].IdexOff;
		XMVECTOR *pVrt = pBuf->pGBSys + Grp[g].VertOff;

		XMMATRIX mWI = XMMatrixInverse(nullptr, mW);

		XMVECTOR pos = XMVector3TransformCoord(Zero, mWI);
		XMVECTOR dir = XMVector3TransformNormal(Dir, mWI);		
		...
		for (DWORD i=0;i<Grp[g].nFace;i++) {
			...
			cp = XMVector3Cross(XMVectorSubtract(_c, _b), XMVectorSubtract(_a, _b));
			float dp = XMVectorGetX(XMVector3Dot(cp, dir));

			if ((dp < 0) || p->bDualSided) {
				if (DirectX::TriangleTests::Intersects(pos, dir, _a, _b, _c, dst)) {
					if (dst > p->fnear) {
						if (dst < result.dist) {
							result.dist = dst;
							result.group = int(g);
...
//
void vkMesh::RenderAxisVector(FMATRIX4* pW, const FVECTOR4 *pColor, float len)
{
	UINT numPasses = 0;
...
//
void vkMesh::RenderRings(const FMATRIX4* pW, LPDIRECT3DTEXTURE9 pTex)
{
	_TRACE;
	if (!IsOK()) return;
	if (!pTex) return;

	vkStats.Mesh.Vertices += Grp[0].nVert;
	vkStats.Mesh.MeshGrps++;
	...
	FX->SetValue(eSun, &sunLight, sizeof(vkSun));
	HR(FX->SetValue(eMtrl, &defmat, sizeof(vkMatExt)-4));
	HR(FX->Begin(&numPasses, D3DXFX_DONOTSAVESTATE));
	HR(FX->BeginPass(0));
	RenderGroup(0);
	HR(FX->EndPass());
...
// Used only by ring manager --------------------------------------------------------------------
//
void vkMesh::RenderRings2(const FMATRIX4* pW, LPDIRECT3DTEXTURE9 pTex, float irad, float orad)
{
	_TRACE;
	if (!IsOK()) return;
	if (!pTex) return;
	...
	FX->SetValue(eSun, &sunLight, sizeof(vkSun));
	HR(FX->SetValue(eMtrl, &defmat, sizeof(vkMatExt)-4));
	HR(FX->SetVector(eTexOff, _DX(FVECTOR4(irad, orad, 0, 0))));
	HR(FX->Begin(&numPasses, D3DXFX_DONOTSAVESTATE));
	HR(FX->BeginPass(0));
	RenderGroup(0);
	HR(FX->EndPass());
...
// ===========================================================================================
//
void vkMesh::GlobalInit(LPDIRECT3DDEVICE9 pDev)
{
	memset(s_pShader, 0, sizeof(s_pShader));

	s_pShader[SHADER_SHADOWMAP] = new MeshShader(pDev, "Modules/vkShaders/NewMesh.hlsl", "ShdMapVS", "ShdMapPS");
	s_pShader[SHADER_SHADOWMAP_OIT] = new MeshShader(pDev, "Modules/vkShaders/NewMesh.hlsl", "ShdMapOIT_VS", "ShdMapOIT_PS");
	s_pShader[SHADER_NORMAL_DEPTH] = new MeshShader(pDev, "Modules/vkShaders/NewMesh.hlsl", "NormalDepth_VS", "NormalDepth_PS");
}


// ===========================================================================================
//
void vkMesh::GlobalExit()
{
